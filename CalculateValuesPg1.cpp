...
void CCalculateValuesPg1::OnBRenumber()
{
	...
	if (UpdateData(TRUE))
	{
		...
		CBasicObj basicCheck;
		if (basicCheck.Renumber((*itemIter).calcValue.m_listCommands))
		{
			// update grid
			DisplayTab(nRate);
		}
...
BOOL CCalculateValuesPg1::OnHelpInfo(HELPINFO* pHelpInfo)
{
...
void CCalculateValuesPg1::OnSetfocusTreeArgs(NMHDR* pNMHDR, LRESULT* pResult)
{
	UNUSED(pNMHDR);
	CString strRes;
	strRes.LoadString(IDS_RATE_260);
