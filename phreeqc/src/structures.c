...
static struct solution *solution_copy(struct solution *solution_old_ptr, int n_user_new);
 ...
 */
	...
	moles_per_kilogram_string = free_check_null(moles_per_kilogram_string);
	pe_string = free_check_null(pe_string);
/* model */
...
/* master species */
	...
	master = free_check_null(master);

/* elements */

	for (j=0; j<count_elements; j++) {
		elements[j] = free_check_null(elements[j]);
	}
	elements = free_check_null(elements);
...
/* solutions */
	...
	solution = free_check_null(solution);
...
/* surfaces */
	...
	surface = free_check_null(surface);
...
/* exchange */
	...
	exchange = free_check_null(exchange);
...
/* pp assemblages */
	...
	pp_assemblage = free_check_null(pp_assemblage);
...
/* s_s assemblages */
	...
	s_s_assemblage = free_check_null(s_s_assemblage);
...
/* temperature */
	...
	temperature = free_check_null(temperature);
...
/* mixtures */
	...
	mix = free_check_null (mix);

/* phases */

	for (j = 0; j < count_phases; j++) {
		phase_free(phases[j]);
		phases[j] = free_check_null(phases[j]);
	}
	phases = free_check_null (phases);

/* inverse */
	for (j = 0; j < count_inverse; j++) {
		inverse_free(&(inverse[j]));
	}
	inverse = free_check_null(inverse);

/* gases */
	for (j = 0; j < count_gas_phase; j++) {
		gas_phase_free(&gas_phase[j]);
	};
	gas_phase = free_check_null(gas_phase);

/* kinetics */
	for (j = 0; j < count_kinetics; j++) {
		kinetics_free(&kinetics[j]);
	}
	kinetics = free_check_null(kinetics);

/* rates */
	for (j = 0; j < count_rates; j++) {
		rate_free(&rates[j]);
	}
	rates = free_check_null(rates);
...
/* save_values */
	for (j = 0; j < count_save_values; j++) {
		save_values[j].subscripts = free_check_null(save_values[j].subscripts);
	}
...
/* punch */
	...
	punch.totals = free_check_null(punch.totals);
	punch.molalities = free_check_null(punch.molalities);
	punch.activities = free_check_null(punch.activities);
	punch.pure_phases = free_check_null(punch.pure_phases);
	punch.si = free_check_null(punch.si);
	punch.gases = free_check_null(punch.gases);
	punch.s_s = free_check_null(punch.s_s);
	punch.kinetics = free_check_null(punch.kinetics);
	advection_punch = free_check_null(advection_punch);
	advection_print = free_check_null(advection_print);
	punch.isotopes = free_check_null(punch.isotopes);
	punch.calculate_values = free_check_null(punch.calculate_values);

/*  user_print and user_punch */
	rate_free(user_print);
	rate_free(user_punch);
	user_print = free_check_null(user_print);

	user_punch = free_check_null(user_punch);
	user_punch_headings = free_check_null(user_punch_headings);
...
/* miscellaneous work space */
	...
	elt_list = free_check_null(elt_list);
	trxn.token = free_check_null(trxn.token);
	mb_unknowns = free_check_null(mb_unknowns);
...
/* free user database name if defined */
	user_database = free_check_null(user_database);
 	selected_output_file_name = free_check_null(selected_output_file_name);
 	dump_file_name = free_check_null(dump_file_name);
#ifdef PHREEQCI_GUI
	free_spread();
 ...
 */
	...
	if (elements[count_elements] == NULL) malloc_error();
 ...
 */
	if (elt_list_ptr_old == NULL) return (NULL);
	for (i = 0; elt_list_ptr_old[i].elt != NULL; i++);
	count_totals = i;
/*
 *   Malloc space and store element data
 */
	elt_list_ptr_new = (struct elt_list *) PHRQ_malloc( (size_t) (count_totals + 1)
						  * sizeof(struct elt_list));
	if (elt_list_ptr_new == NULL ) malloc_error();
 ...
 */
	if (elt_list_ptr == NULL) return (ERROR);
	output_msg(OUTPUT_MESSAGE, "Elt_list\n");
	for (i = 0; elt_list_ptr[i].elt != NULL; i++) {
		output_msg(OUTPUT_MESSAGE, "\t%s\t%e\n", elt_list_ptr[i].elt->name, (double) elt_list_ptr[i].coef);
	}
	return (OK);
}
/* ---------------------------------------------------------------------- */
int elt_list_print0(void)
/* ---------------------------------------------------------------------- */
{
/*
 *  Duplicates the elt_list structure pointed to by elt_list_ptr_old.
 ...
 *   Malloc space and store element data
 */
	elt_list_ptr=(struct elt_list *) PHRQ_malloc((size_t) (count_elts + 1)*sizeof(struct elt_list));
...
/* ---------------------------------------------------------------------- */
{
	...
	if (exchange_ptr == NULL) malloc_error();
...
/* ---------------------------------------------------------------------- */
{
	...
	return(strcmp_nocase(exch_comp_ptr1->master->elt->name, exch_comp_ptr2->master->elt->name));
}
/* ---------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------- */
{
	...
	if (exchange_ptr1->n_user > exchange_ptr2->n_user) return(1);
	if (exchange_ptr1->n_user < exchange_ptr2->n_user) return(-1);
	return (0);
 ...
 */
	exchange_new_ptr->comps = (struct exch_comp *) PHRQ_malloc((size_t) (count_comps) * sizeof (struct exch_comp));
	if (exchange_new_ptr->comps == NULL) malloc_error();
/*
 ...
 */
	...
	for ( i = 0; i < count_comps; i++) {
		exchange_new_ptr->comps[i].totals = elt_list_dup(exchange_old_ptr->comps[i].totals);
		exchange_new_ptr->comps[i].formula_totals = elt_list_dup(exchange_old_ptr->comps[i].formula_totals);
	}
	return(OK);
}
/* ---------------------------------------------------------------------- */
 ...
 */
	if (n_user_old == n_user_new) return(OK);
 ...
 */
	int i;
	if (exchange_ptr == NULL) return(ERROR);
/*
 *   Free space allocated for exchange structure
 */
	exchange_ptr->description = free_check_null (exchange_ptr->description);
	for (i=0; i < exchange_ptr->count_comps; i++) {
		exchange_ptr->comps[i].totals = free_check_null (exchange_ptr->comps[i].totals);
		exchange_ptr->comps[i].formula_totals = free_check_null (exchange_ptr->comps[i].formula_totals);
	}
	exchange_ptr->comps = free_check_null (exchange_ptr->comps);
	return(OK);
}
/* ---------------------------------------------------------------------- */
 ...
 */
	...
	exchange_ptr->count_comps = 0;
	exchange_ptr->comps = (struct exch_comp *) PHRQ_malloc(sizeof (struct exch_comp));
 ...
 */
	if (count_exchange > 0) {
		qsort (exchange, (size_t) count_exchange, (size_t) sizeof(struct exchange),
		       exchange_compare);
	}
	return(OK);
}
...
/* ---------------------------------------------------------------------- */
{
	...
	gas_phase_ptr = (struct gas_phase *) PHRQ_malloc(sizeof (struct gas_phase));
	if (gas_phase_ptr == NULL) malloc_error();
...
/* ---------------------------------------------------------------------- */
{
	...
	if (gas_phase_ptr1->n_user > gas_phase_ptr2->n_user) return(1);
	if (gas_phase_ptr1->n_user < gas_phase_ptr2->n_user) return(-1);
	return (0);
 ...
 */
	memcpy(gas_phase_new_ptr, gas_phase_old_ptr, (size_t) sizeof(struct gas_phase));
 ...
 */
	gas_phase_new_ptr->comps = (struct gas_comp *) PHRQ_malloc((size_t) (count_comps) * sizeof (struct gas_comp));
	if (gas_phase_new_ptr->comps == NULL) malloc_error();
/*
 ...
 */
	if (gas_phase_ptr == NULL) return(ERROR);
/*
 *   Free space allocated for gas_phase structure
 */
	gas_phase_ptr->description = free_check_null (gas_phase_ptr->description);
	gas_phase_ptr->comps = free_check_null (gas_phase_ptr->comps);
	return(OK);
}
/* ---------------------------------------------------------------------- */
 ...
 */
	...
	gas_phase_ptr->count_comps = 0;
	gas_phase_ptr->comps = PHRQ_malloc((size_t) sizeof(struct gas_comp));
 ...
 */
	if (count_gas_phase > 0) {
		qsort (gas_phase, (size_t) count_gas_phase, (size_t) sizeof(struct gas_phase),
		       gas_phase_compare);
	}
	return(OK);
}
 ...
 */
{
	...
	count_inverse++;
	inverse = (struct inverse *) PHRQ_realloc(inverse, (size_t) count_inverse * sizeof(struct inverse));
 ...
 */
	...
	inverse_ptr->uncertainties = PHRQ_malloc((size_t) sizeof(LDBLE));
	if (inverse_ptr->uncertainties == NULL) malloc_error();

	inverse_ptr->ph_uncertainties = PHRQ_malloc((size_t) sizeof(LDBLE));
	if (inverse_ptr->ph_uncertainties == NULL) malloc_error();

#ifdef SKIP
	inverse_ptr->alk_uncertainties = PHRQ_malloc((size_t) sizeof(LDBLE));
	if (inverse_ptr->alk_uncertainties == NULL) malloc_error();
#endif

	inverse_ptr->force_solns = PHRQ_malloc((size_t) sizeof(int));
	if (inverse_ptr->force_solns == NULL) malloc_error();
	...
	inverse_ptr->elts = PHRQ_malloc((size_t) sizeof(struct inv_elts));
	if (inverse_ptr->elts == NULL) malloc_error();
	inverse_ptr->elts[0].name = NULL;
	inverse_ptr->elts[0].uncertainties = NULL;

	inverse_ptr->isotopes = PHRQ_malloc((size_t) sizeof(struct inv_isotope));
	...
	inverse_ptr->i_u = PHRQ_malloc((size_t) sizeof(struct inv_isotope));
	...
	inverse_ptr->phases = PHRQ_malloc((size_t) sizeof(struct inv_phases));
	if (inverse_ptr->phases == NULL) malloc_error();

	return(inverse_ptr);
}
/* ---------------------------------------------------------------------- */
int inverse_compare(const void *ptr1, const void *ptr2)
/* ---------------------------------------------------------------------- */
{
/*
 ...
 */
	...
	if (nptr1->n_user > nptr2->n_user) return(1);
	if (nptr1->n_user < nptr2->n_user) return(-1);
	return (0);
}
/* ---------------------------------------------------------------------- */
 ...
 */
	...
	inverse_ptr->description = free_check_null (inverse_ptr->description);
/*   Free solns */
	inverse_ptr->solns = free_check_null (inverse_ptr->solns);

/*   Free uncertainties */
	inverse_ptr->uncertainties = free_check_null (inverse_ptr->uncertainties);
	inverse_ptr->ph_uncertainties = free_check_null (inverse_ptr->ph_uncertainties); 
#ifdef SKIP
	inverse_ptr->alk_uncertainties = free_check_null (inverse_ptr->alk_uncertainties);
#endif

/*   Free force_solns */
	inverse_ptr->force_solns = free_check_null (inverse_ptr->force_solns); 

/*   Free elts */
	for (i = 0; i < inverse_ptr->count_elts; i++) {
		inverse_ptr->elts[i].uncertainties = free_check_null (inverse_ptr->elts[i].uncertainties);
	};
	inverse_ptr->elts = free_check_null (inverse_ptr->elts);

/*   Free isotopes */
	for (i = 0; i < inverse_ptr->count_isotopes; i++) {
		inverse_ptr->isotopes[i].uncertainties = free_check_null (inverse_ptr->isotopes[i].uncertainties);
	};
	inverse_ptr->isotopes = free_check_null (inverse_ptr->isotopes);

	for (i = 0; i < inverse_ptr->count_i_u; i++) {
		inverse_ptr->i_u[i].uncertainties = free_check_null (inverse_ptr->i_u[i].uncertainties);
	};
	inverse_ptr->i_u = free_check_null (inverse_ptr->i_u);

/*   Free phases */
	for (i = 0; i < inverse_ptr->count_phases; i++) {
		inverse_ptr->phases[i].isotopes = free_check_null(inverse_ptr->phases[i].isotopes);
	}
	inverse_ptr->phases = free_check_null (inverse_ptr->phases);

/*   Free carbon derivatives */
	inverse_ptr->dalk_dph = free_check_null(inverse_ptr->dalk_dph);
	inverse_ptr->dalk_dc = free_check_null(inverse_ptr->dalk_dc);
 ...
 *   Allocate space
 */
	irrev_new_ptr->list = (struct name_coef *) PHRQ_malloc((size_t) (count_list) * sizeof (struct name_coef));
	if (irrev_new_ptr->list == NULL) malloc_error();
	memcpy(irrev_new_ptr->list, irrev_old_ptr->list, (size_t) (count_list) * sizeof (struct name_coef));
	if (count_steps < 0) count_steps = 1;
	irrev_new_ptr->steps = (LDBLE *) PHRQ_malloc((size_t) (count_steps) * sizeof (LDBLE));
	if (irrev_new_ptr->steps == NULL) malloc_error();
	memcpy(irrev_new_ptr->steps, irrev_old_ptr->steps, (size_t) (count_steps) * sizeof (LDBLE));
 ...
 */
	...
	if (irrev_ptr_new != NULL) {
		...
		if (n_user_new < irrev[count_irrev-1].n_user) sort = TRUE;
		n_new = count_irrev++;
	}
/*
 ...
 */
	...
	sprintf(token, "Irreversible reaction defined in simulation %d.", simulation);
 ...
 *   Allocate space
 */
	irrev[n_new].list = (struct name_coef *) PHRQ_malloc((size_t) (count_list) * sizeof (struct name_coef));
	if (irrev[n_new].list == NULL) malloc_error();
	memcpy(irrev[n_new].list, irrev[n_old].list, 
	       (size_t) (count_list) * sizeof (struct name_coef));

	if (count_steps < 0) count_steps = 1;
	irrev[n_new].steps = (LDBLE *) PHRQ_malloc((size_t) (count_steps) * sizeof (LDBLE));
	if (irrev[n_new].steps == NULL) malloc_error();
 ...
 */
	irrev_ptr->description = free_check_null (irrev_ptr->description);
	irrev_ptr->list = free_check_null (irrev_ptr->list);
	irrev_ptr->elts = free_check_null (irrev_ptr->elts);
	irrev_ptr->steps = free_check_null (irrev_ptr->steps);
	return(OK);
}
/* ---------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------- */
{
	...
	if (kinetics_ptr == NULL) malloc_error();
...
/* ---------------------------------------------------------------------- */
{
	...
	if (kinetics_ptr1->n_user > kinetics_ptr2->n_user) return(1);
	if (kinetics_ptr1->n_user < kinetics_ptr2->n_user) return(-1);
	return (0);
 ...
 */
	...
	if (count_steps > 0) {
		kinetics_new_ptr->steps = (LDBLE *) PHRQ_malloc((size_t) (count_steps * sizeof (LDBLE)));
		if (kinetics_new_ptr->steps == NULL) malloc_error();
		memcpy(kinetics_new_ptr->steps, kinetics_old_ptr->steps, (size_t) count_steps * sizeof (LDBLE));
	}
/*
 *   Copy kinetic components
 */
	if (kinetics_new_ptr->count_comps > 0) {
		kinetics_new_ptr->comps = PHRQ_malloc((size_t) kinetics_old_ptr->count_comps * sizeof (struct kinetics_comp));
		if (kinetics_new_ptr->comps == NULL) malloc_error();
	} else {
		kinetics_new_ptr->comps = NULL;
	}
	for (i = 0; i < kinetics_old_ptr->count_comps; i++) {
		kinetics_comp_duplicate(&kinetics_new_ptr->comps[i], &kinetics_old_ptr->comps[i]);
	}
	return(OK);
}
/* ---------------------------------------------------------------------- */
 ...
 */
	memcpy(kinetics_comp_new_ptr, kinetics_comp_old_ptr, sizeof (struct kinetics_comp));
/*
 *   Kinetics character parameters
 */
	if (kinetics_comp_new_ptr->count_c_params > 0) {
		kinetics_comp_new_ptr->c_params = (char **) PHRQ_malloc((size_t) (kinetics_comp_old_ptr->count_c_params) * sizeof (char *));
		if (kinetics_comp_new_ptr->c_params == NULL) malloc_error();
		memcpy(kinetics_comp_new_ptr->c_params, kinetics_comp_old_ptr->c_params, (size_t) (kinetics_comp_old_ptr->count_c_params) * sizeof (char *));
	} else {
 ...
 */
	if (kinetics_comp_new_ptr->count_d_params > 0) {
		kinetics_comp_new_ptr->d_params = (LDBLE *) PHRQ_malloc((size_t) (kinetics_comp_old_ptr->count_d_params) * sizeof (LDBLE));
		if (kinetics_comp_new_ptr->d_params == NULL) malloc_error();
		memcpy(kinetics_comp_new_ptr->d_params, kinetics_comp_old_ptr->d_params, (size_t) (kinetics_comp_old_ptr->count_d_params) * sizeof (LDBLE));
	} else {
		kinetics_comp_new_ptr->d_params = (LDBLE *) PHRQ_malloc((size_t) sizeof (LDBLE));
	}
/*
 *   Kinetics list of formulae
 */
	if (kinetics_comp_new_ptr->count_list > 0) {
		kinetics_comp_new_ptr->list = (struct name_coef *) PHRQ_malloc((size_t) (kinetics_comp_old_ptr->count_list) * sizeof (struct name_coef));
		if (kinetics_comp_new_ptr->list == NULL) malloc_error();
		memcpy(kinetics_comp_new_ptr->list, kinetics_comp_old_ptr->list, (size_t) (kinetics_comp_old_ptr->count_list) * sizeof (struct name_coef));
	} else {
 ...
 */
	for (i = 0; i < kinetics_ptr->count_comps; i++) {
		kinetics_ptr->comps[i].c_params = free_check_null (kinetics_ptr->comps[i].c_params);
		kinetics_ptr->comps[i].d_params = free_check_null (kinetics_ptr->comps[i].d_params);
		kinetics_ptr->comps[i].list = free_check_null (kinetics_ptr->comps[i].list);
	}
	kinetics_ptr->description = free_check_null (kinetics_ptr->description);
	kinetics_ptr->steps = free_check_null (kinetics_ptr->steps);
	kinetics_ptr->comps = free_check_null (kinetics_ptr->comps);
...
/* ---------------------------------------------------------------------- */
{
	...
	kinetics_ptr->count_comps = 0;
	kinetics_ptr->comps = PHRQ_malloc((size_t) sizeof(struct kinetics_comp));
	if (kinetics_ptr->comps == NULL) malloc_error();
	kinetics_ptr->count_steps = 0;
	kinetics_ptr->steps = PHRQ_malloc((size_t) sizeof(LDBLE));
 ...
 */
	if (count_kinetics > 0) {
		qsort (kinetics, (size_t) count_kinetics, (size_t) sizeof(struct kinetics),
		       kinetics_compare);
	}
	return(OK);
}
 ...
 */
	...
	if (master_search(ptr, &n) == NULL) return(FALSE);
 ...
 */
	if (master_ptr == NULL) return(ERROR);
	rxn_free (master_ptr->rxn_primary);
	rxn_free (master_ptr->rxn_secondary);
	master_ptr = free_check_null (master_ptr);
	return(OK);
}
...
/* ---------------------------------------------------------------------- */
int master_compare_string (const void *ptr1, const void *ptr2)
/* ---------------------------------------------------------------------- */
{
 ...
 */
	...
	if (master_ptr_primary == NULL ) {
		input_error++;
		sprintf(error_string, "Could not find primary master species for %s.", ptr);
		error_msg(error_string, CONTINUE);
	}
...
/* ---------------------------------------------------------------------- */
{
	...
	if (mix_ptr1->n_user > mix_ptr2->n_user) return(1);
	if (mix_ptr1->n_user < mix_ptr2->n_user) return(-1);
	return (0);
 ...
 */
	...
	if (mix_ptr_new != NULL) {
		...
		if (n_user_new < mix[count_mix - 1].n_user) sort = TRUE;
		n_new = count_mix++;
	}
/*
 ...
 */
	mix[n_new].comps = PHRQ_malloc((size_t) (count_comps) * sizeof (struct mix_comp));
	if (mix[n_new].comps == NULL) malloc_error();
/*
 ...
 */
	...
	mix_ptr->description = free_check_null (mix_ptr->description);
	mix_ptr->comps = free_check_null (mix_ptr->comps);
	return(OK);
}
/* ---------------------------------------------------------------------- */
 ...
 */
	...
	pe_data_ptr = (struct pe_data *) PHRQ_malloc( (size_t) (2 * sizeof (struct pe_data)));
	if (pe_data_ptr == NULL) malloc_error();
	...
	if (s_eminus != NULL && s_eminus->rxn != NULL) {
		...
		if (pe_data_ptr[0].rxn == NULL) malloc_error();
		strcpy(token,"e-");
 ...
 */
	if (pe_ptr_old == NULL) return(ERROR);
	for (i = 0; pe_ptr_old[i].name != NULL; i++);
	count_pe = i + 1;
	pe_ptr_new = PHRQ_malloc( (size_t) count_pe * sizeof(struct pe_data));
	if (pe_ptr_new == NULL) malloc_error();
	memcpy ( (void *) pe_ptr_new,  (void *) pe_ptr_old, (size_t) count_pe * sizeof(struct pe_data) );
 ...
 */
	...
	if ( pe_data_ptr == NULL ) return(ERROR);
	for (i=0; pe_data_ptr[i].name != NULL; i++) {
		rxn_free(pe_data_ptr[i].rxn);
	}
	pe_data_ptr = free_check_null (pe_data_ptr);
	return(OK);
}
/* ---------------------------------------------------------------------- */
 ...
 */
	*pe = PHRQ_realloc( (void *) *pe, (size_t) (i+2) * sizeof(struct pe_data));
	if (*pe == NULL) malloc_error();
	pe_data_ptr = *pe;
 ...
 */
	...
	if (phase_ptr == NULL) malloc_error();
/*
 *   Initialize space
 */
...
/* ---------------------------------------------------------------------- */
int phase_compare_string (const void *ptr1, const void *ptr2)
/* ---------------------------------------------------------------------- */
{
	const char *char_ptr;
 ...
 */
	...
	phase_free(phases[i]);
	phases[i] = free_check_null(phases[i]);
 ...
 *   Return: OK
 */
	if (phase_ptr == NULL) return(ERROR);
	phase_ptr->next_elt = free_check_null (phase_ptr->next_elt);
	phase_ptr->next_sys_total = free_check_null (phase_ptr->next_sys_total); 
	rxn_free (phase_ptr->rxn);
	rxn_free (phase_ptr->rxn_s);
	rxn_free (phase_ptr->rxn_x);
	phase_ptr->add_logk = free_check_null (phase_ptr->add_logk);
	return(OK);
}
/* ---------------------------------------------------------------------- */
 ...
 */
{
	...
	phase_ptr->in = FALSE;
	phase_ptr->lk = 0.0;
	for (i = 0; i < 8; i++) phase_ptr->logk[i]=0.0;
...
#ifdef SKIP
/* ---------------------------------------------------------------------- */
struct phase *phase_search (char *ptr, int *n)
/* ---------------------------------------------------------------------- */
{
/*
...
/* ---------------------------------------------------------------------- */
{
	...
	pp_assemblage_ptr = (struct pp_assemblage *) PHRQ_malloc(sizeof (struct pp_assemblage));
	if (pp_assemblage_ptr == NULL) malloc_error();
...
/* ---------------------------------------------------------------------- */
{
	...
	if (pp_assemblage_ptr1->n_user > pp_assemblage_ptr2->n_user) return(1);
	if (pp_assemblage_ptr1->n_user < pp_assemblage_ptr2->n_user) return(-1);
	return (0);
...
int pp_assemblage_copy(struct pp_assemblage *pp_assemblage_old_ptr, 
			      struct pp_assemblage *pp_assemblage_new_ptr, int n_user_new)
/* ---------------------------------------------------------------------- */
{
/*
 ...
 */
	...
	pp_assemblage_new_ptr->next_elt = elt_list_dup(pp_assemblage_old_ptr->next_elt);
/*
 *   Count pure phases
 */
 ...
 */
	pp_assemblage_new_ptr->pure_phases = (struct pure_phase *) PHRQ_malloc( (size_t) count_comps * sizeof(struct pure_phase));
	if (pp_assemblage_new_ptr->pure_phases == NULL) malloc_error();
 ...
 *   Copy data
 */
	pp_assemblage_copy(&pp_assemblage[n_old], &pp_assemblage[n_new], n_user_new);
 ...
 */
	if (pp_assemblage_ptr == NULL) return(ERROR);

	pp_assemblage_ptr->description = free_check_null (pp_assemblage_ptr->description);
	pp_assemblage_ptr->next_elt = free_check_null (pp_assemblage_ptr->next_elt);
	pp_assemblage_ptr->pure_phases = free_check_null (pp_assemblage_ptr->pure_phases);
	return(OK);
}
/* ---------------------------------------------------------------------- */
 ...
 */
	...
	pp_assemblage_ptr->next_elt = NULL;
	pp_assemblage_ptr->count_comps = 0;
	pp_assemblage_ptr->pure_phases = PHRQ_malloc((size_t) sizeof(struct pure_phase));
 ...
 */
	if (count_pp_assemblage > 0) {
		qsort (pp_assemblage, (size_t) count_pp_assemblage, (size_t) sizeof(struct pp_assemblage),
...
/* ---------------------------------------------------------------------- */
int rate_compare (const void *ptr1, const void *ptr2)
/* ---------------------------------------------------------------------- */
{
/*
...
/* ---------------------------------------------------------------------- */
int rate_compare_string (const void *ptr1, const void *ptr2)
/* ---------------------------------------------------------------------- */
{
	const char *char_ptr;
 ...
 */
	...
	if (rate_ptr == NULL) return(ERROR);
	rate_ptr->commands = free_check_null(rate_ptr->commands);
 ...
 */
	...
	if (rxn_ptr_old == NULL) return(NULL);
 ...
 *   Copy tokens
 */
	memcpy (rxn_ptr_new->token, rxn_ptr_old->token, (size_t) (i + 1) * sizeof (struct rxn_token));
 ...
 */
	...
	rxn_ptr->token = free_check_null (rxn_ptr->token);
	rxn_ptr = free_check_null (rxn_ptr);
	return(OK);
}
/* **********************************************************************
 *
 ...
 */
	if (s_ptr == NULL) return(ERROR);
	s_ptr->next_elt = free_check_null (s_ptr->next_elt);
	s_ptr->next_secondary = free_check_null (s_ptr->next_secondary);
	s_ptr->next_sys_total = free_check_null (s_ptr->next_sys_total);
	s_ptr->add_logk = free_check_null(s_ptr->add_logk);
	rxn_free (s_ptr->rxn);
	rxn_free (s_ptr->rxn_s);
	rxn_free (s_ptr->rxn_x);
	s_ptr->diff_layer = free_check_null (s_ptr->diff_layer);
	return(OK);
}
/* ---------------------------------------------------------------------- */
static int s_init(struct species *s_ptr)
...
/* ---------------------------------------------------------------------- */
{
	...
	s_s_assemblage_ptr = (struct s_s_assemblage *) PHRQ_malloc(sizeof (struct s_s_assemblage));
	if (s_s_assemblage_ptr == NULL) malloc_error();
...
/* ---------------------------------------------------------------------- */
{
	...
	if (s_s_assemblage_ptr1->n_user > s_s_assemblage_ptr2->n_user) return(1);
	if (s_s_assemblage_ptr1->n_user < s_s_assemblage_ptr2->n_user) return(-1);
	return (0);
 ...
 */
	memcpy(s_s_assemblage_new_ptr, s_s_assemblage_old_ptr, sizeof(struct s_s_assemblage));
 ...
 */
	...
	s_s_assemblage_new_ptr->s_s = (struct s_s *) PHRQ_malloc( (size_t) count_s_s * sizeof(struct s_s));
	if (s_s_assemblage_new_ptr->s_s == NULL) malloc_error();
 ...
 */
	for (i = 0; i < count_s_s; i++) {
		...
		s_s_assemblage_new_ptr->s_s[i].comps = (struct s_s_comp *) PHRQ_malloc( (size_t) count_comps * sizeof(struct s_s_comp));
		if (s_s_assemblage_new_ptr->s_s[i].comps == NULL) malloc_error();
 ...
 *   Copy data
 */
	s_s_assemblage_copy(&s_s_assemblage[n_old], &s_s_assemblage[n_new], n_user_new);
 ...
 */
	...
	if (s_s_assemblage_ptr == NULL) return(ERROR);
	s_s_assemblage_ptr->description = free_check_null (s_s_assemblage_ptr->description);
	for (i = 0; i < s_s_assemblage_ptr->count_s_s; i++) {
		s_s_assemblage_ptr->s_s[i].comps = free_check_null(s_s_assemblage_ptr->s_s[i].comps);
	}
	s_s_assemblage_ptr->s_s = free_check_null (s_s_assemblage_ptr->s_s);
	return(OK);
}
/* ---------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------- */
{
	...
	s_s_assemblage_ptr->count_s_s = 0;
	s_s_assemblage_ptr->s_s = PHRQ_malloc((size_t) sizeof(struct s_s));
 ...
 */
	if (count_s_s_assemblage > 0) {
		qsort (s_s_assemblage, (size_t) count_s_s_assemblage, (size_t) sizeof(struct s_s_assemblage),
 ...
 */
	...
	void_ptr = (void *)
		bsearch ( (char *) k,
			  ...
			  (size_t) sizeof(struct save_values),
			  save_values_compare);
	if (void_ptr == NULL) {
		*n = -999;
		return(NULL);
...
/* ---------------------------------------------------------------------- */
{
	...
	if (save_values_ptr1->count_subscripts < save_values_ptr2->count_subscripts) {
		return(-1);
	} else if (save_values_ptr1->count_subscripts > save_values_ptr2->count_subscripts) {
		return(1);
	} else {
		for (i=0; i < save_values_ptr1->count_subscripts ;i++) {
			if (save_values_ptr1->subscripts[i] < save_values_ptr2->subscripts[i]) {
				return(-1);
			} else if (save_values_ptr1->subscripts[i] > save_values_ptr2->subscripts[i]) {
				return(1);
			} 
		}
	}
 ...
 */
	if (count_save_values > 0) {
		qsort (save_values, (size_t) count_save_values, (size_t) sizeof(struct save_values),
		       save_values_compare);
	}
	return(OK);
}
 ...
 */
	...
	if (s_v_ptr != NULL) {
		...
		save_values = PHRQ_realloc(save_values, (size_t) (count_save_values + 1) * sizeof( struct save_values));
		...
		if (i == 0) i = 1;
		save_values[count_save_values].subscripts = PHRQ_malloc ((size_t) i * sizeof(int));
		if (save_values[count_save_values].subscripts == NULL) malloc_error();
		save_values[count_save_values].subscripts = memcpy(save_values[count_save_values].subscripts, s_v->subscripts, (size_t) i * sizeof(int));
		count_save_values++;
		save_values_sort();
	}
 ...
 */
	...
	space ((void *) &(solution_ptr->totals), INIT, &max_mass_balance, sizeof (struct conc));
	max_mass_balance=MAX_MASS_BALANCE;
 ...
 */
	solution_ptr->isotopes = PHRQ_malloc(sizeof(struct isotope));
...
/* ---------------------------------------------------------------------- */
int solution_compare(const void *ptr1, const void *ptr2)
/* ---------------------------------------------------------------------- */
{
/*
...
/* ---------------------------------------------------------------------- */
int solution_compare_int(const void *ptr1, const void *ptr2)
/* ---------------------------------------------------------------------- */
{
/*
 ...
 */
	solution_new_ptr = (struct solution *) PHRQ_malloc(sizeof(struct solution));
	if (solution_new_ptr == NULL) malloc_error();
/*
 *   Copy solution data, but set pointers pe and totals to NULL
 */
	memcpy ( (void *) solution_new_ptr, (void *) solution_old_ptr, (size_t) sizeof (struct solution));
	solution_new_ptr->n_user = n_user_new;
	solution_new_ptr->n_user_end = n_user_new;
	solution_new_ptr->description = string_duplicate(solution_old_ptr->description);
/*
 *   Count totals data and malloc space
 */
	for (i = 0; solution_old_ptr->totals[i].description != NULL; i++);
	count_totals = i + 1;
	solution_new_ptr->totals = (struct conc *) PHRQ_malloc( (size_t) count_totals * sizeof(struct conc));
	if (solution_new_ptr->totals == NULL) malloc_error();
/*
 *   Copy totals data
 */
	memcpy ( (void *) solution_new_ptr->totals, (void *) solution_old_ptr->totals, (size_t) count_totals * sizeof (struct conc));
/*
 ...
 */
	if (solution_old_ptr->count_isotopes > 0) {
		...
		solution_new_ptr->isotopes = PHRQ_malloc((size_t) solution_old_ptr->count_isotopes * sizeof(struct isotope));
		if (solution_new_ptr->isotopes == NULL) malloc_error();
		memcpy(solution_new_ptr->isotopes, solution_old_ptr->isotopes, (size_t) solution_old_ptr->count_isotopes * sizeof(struct isotope));
	} else {
 ...
 */
	if (nptr1->master_s != nptr2->master_s) {
		if (nptr1->master_s == s_hplus) return (-1);
		if (nptr2->master_s == s_hplus) return (1);
	}
/*
...
/* ---------------------------------------------------------------------- */
{
	...
	if (surface_ptr == NULL) malloc_error();
...
/* ---------------------------------------------------------------------- */
{
	...
	return(strcmp_nocase(surface_comp_ptr1->master->elt->name, surface_comp_ptr2->master->elt->name));
}
/* ---------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------- */
{
	...
	if (surface_ptr1->n_user > surface_ptr2->n_user) return(1);
	if (surface_ptr1->n_user < surface_ptr2->n_user) return(-1);
	return (0);
 ...
 */
	surface_new_ptr->comps = (struct surface_comp *) PHRQ_malloc((size_t) (count_comps) * sizeof (struct surface_comp));
	if (surface_new_ptr->comps == NULL) malloc_error();
 ...
 */
	if (surface_old_ptr->edl == TRUE) {
		...
		for ( i = 0; i < count_charge; i++) {
			surface_new_ptr->charge[i].diffuse_layer_totals = elt_list_dup(surface_old_ptr->charge[i].diffuse_layer_totals);
 ...
 */
	...
	if (surface_ptr == NULL) return(ERROR);
	surface_ptr->description = free_check_null(surface_ptr->description);
/*
 *   totals, then comps
 */
	for (k = 0; k < surface_ptr->count_comps; k++) {
		surface_ptr->comps[k].totals = free_check_null(surface_ptr->comps[k].totals);
	}
	surface_ptr->comps = free_check_null (surface_ptr->comps);
/*
 *   diffuse_layer_totals and g, then charge
 */
	if (surface_ptr->edl == TRUE) {
		for (k = 0; k < surface_ptr->count_charge; k++) {
			surface_ptr->charge[k].diffuse_layer_totals = free_check_null(surface_ptr->charge[k].diffuse_layer_totals);
			surface_ptr->charge[k].g = free_check_null(surface_ptr->charge[k].g);
		}
	}
	surface_ptr->charge = free_check_null (surface_ptr->charge);
	return(OK);
}
/* ---------------------------------------------------------------------- */
 ...
 */
	surface_ptr->count_comps = 0;
	surface_ptr->comps = (struct surface_comp *) PHRQ_malloc( (size_t) sizeof (struct surface_comp));
	if (surface_ptr->comps == NULL) malloc_error();
 ...
 */
	surface_ptr->count_charge = 0;
	surface_ptr->charge = (struct surface_charge *) PHRQ_malloc( (size_t) sizeof (struct surface_charge));
	if (surface_ptr->charge == NULL) malloc_error();
...
/* ---------------------------------------------------------------------- */
int temperature_compare (const void *ptr1, const void *ptr2)
/* ---------------------------------------------------------------------- */
{
	...
	if (temperature_ptr1->n_user > temperature_ptr2->n_user) return(1);
	if (temperature_ptr1->n_user < temperature_ptr2->n_user) return(-1);
	return (0);
 ...
 */
	...
	if (temperature_ptr_new != NULL) {
		...
		temperature = (struct temperature *) PHRQ_realloc( temperature, 
							     (size_t) (count_temperature + 1) *
							     sizeof (struct temperature));
		if (temperature == NULL) malloc_error();
		if (n_user_new < temperature[count_temperature-1].n_user) sort = TRUE;
 ...
 */
	...
	temperature_ptr->description = free_check_null (temperature_ptr->description);
	temperature_ptr->t = free_check_null (temperature_ptr->t);
	return(OK);
}
/* ---------------------------------------------------------------------- */
 ...
 */
	if (count_temperature > 0) {
		qsort (temperature, (size_t) count_temperature, (size_t) sizeof(struct temperature), temperature_compare);
	}
	return(OK);
}
/* **********************************************************************
 ...
 * ********************************************************************** */
/* ---------------------------------------------------------------------- */
int rxn_token_temp_compare (const void *ptr1, const void *ptr2)
/* ---------------------------------------------------------------------- */
{
 ...
 */
	for (j=0; j < count_trxn; j++) {
		if (strcmp(trxn.token[j].s->name,token) == 0 ) break;
	}
	if (j >= count_trxn ) {
		input_error++;
		...
		error_msg(error_string, CONTINUE);
		for (i=0; i<count_trxn; i++) {
			output_msg(OUTPUT_MESSAGE,"%f\t%s\t",(double) trxn.token[i].coef,trxn.token[i].name);
		}
		output_msg(OUTPUT_MESSAGE,"\n");
		return(ERROR);
	}
 ...
 */
	...
	if (unknown_ptr == NULL) malloc_error();
/*
 *   set pointers in structure to NULL
 */
	unknown_ptr->type = 0;
 ...
 */
	...
	unknown_ptr->master = free_check_null (unknown_ptr->master);
	unknown_ptr = free_check_null (unknown_ptr);
	return(OK);
}
/* ---------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------- */
{
	...
	if (solution_bsearch(i,&n, TRUE) != NULL) solution_duplicate(i, save_old);
	if (pp_assemblage_bsearch(i,&n) != NULL) pp_assemblage_duplicate(i, save_old);
	if (exchange_bsearch(i,&n) != NULL) exchange_duplicate(i, save_old);
	if (surface_bsearch(i,&n) != NULL) surface_duplicate(i, save_old);
	if (gas_phase_bsearch(i,&n) != NULL) gas_phase_duplicate(i, save_old);
	if (kinetics_bsearch(i,&n) != NULL) kinetics_duplicate(i, save_old);
	if (s_s_assemblage_bsearch(i,&n) != NULL) s_s_assemblage_duplicate(i, save_old);
	return(OK);
}
/* ---------------------------------------------------------------------- */
 ...
 */
	...
	switch (type) {
	    case UnKnown:
		    warning_msg("EXISTS expecting keyword solution, mix, kinetics, reaction, reaction_temperature, equilibrium_phases, exchange, surface, gas_phase, or solid_solutions.");
		    return_value = 2;
		    break;
 ...
 */
{
	...
	if (copier_ptr->count >= copier_ptr->max) {
		...
		copier_ptr->n_user = PHRQ_realloc(copier_ptr->n_user, (size_t) (copier_ptr->max*sizeof(int)));
		if (copier_ptr->n_user == NULL) malloc_error();
		copier_ptr->start = PHRQ_realloc(copier_ptr->start, (size_t) (copier_ptr->max*sizeof(int)));
		if (copier_ptr->start == NULL) malloc_error();
		copier_ptr->end = PHRQ_realloc(copier_ptr->end, (size_t) (copier_ptr->max*sizeof(int)));
		if (copier_ptr->end == NULL) malloc_error();
	}
 ...
 */
{
	...
	copier_ptr->n_user = free_check_null(copier_ptr->n_user);
	copier_ptr->start = free_check_null(copier_ptr->start);
	copier_ptr->end = free_check_null(copier_ptr->end);
	return(OK);
}
/* ---------------------------------------------------------------------- */
 ...
 */
{
	...
	copier_ptr->count = 0;
	copier_ptr->max = 10;
	copier_ptr->n_user = PHRQ_malloc((size_t) (copier_ptr->max*sizeof(int)));
	copier_ptr->start = PHRQ_malloc((size_t) (copier_ptr->max*sizeof(int)));
	copier_ptr->end = PHRQ_malloc((size_t) (copier_ptr->max*sizeof(int)));
	return(OK);
}
