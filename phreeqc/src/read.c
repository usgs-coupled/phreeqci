...
STATIC int read_delta_h_only (char *ptr, LDBLE *delta_h, DELTA_H_UNIT *units);
...
STATIC int read_line_doubles(char *next_char, LDBLE **d, int *count_d, int *count_alloc);
STATIC int read_lines_doubles(char *next_char, LDBLE **d, int *count_d, int *count_alloc, const char **opt_list, int count_opt_list, int *opt);
  ...
  */
	for (;;) {
		if (next_keyword >= 0) {
			...
			if (next_keyword != 49 && !reading_database()) first_read_input = FALSE;
		}
		switch(next_keyword) {
		case -1:                        /* Have not read line with keyword */
			sprintf(error_string, "Unknown input, no keyword has been specified.");
			warning_msg(error_string);
			while ((j = check_line ("No keyword", FALSE, TRUE, TRUE, TRUE)) != KEYWORD 
			       && j != EOF ) {
				warning_msg(error_string);
			}
			break;
 ...
 */
	...
	while ( ((j = copy_token(token, &ptr, &l)) == UPPER ) ||
		...
		count_redox_states++;
		replace("(+","(",token);
		if (count_redox_states > 1) strcat(token1," ");
		strcat(token1,token);
	}
	if (count_redox_states == 0) {
		input_error++;
		error_msg ("No element or master species given for concentration input.", CONTINUE);
		return(ERROR);
	}
 ...
 */
	...
	j=sscanf(token,SCANFORMAT, &(solution[n]->totals[count_mass_balance].input_conc));
	if (j == 0 ) {
		sprintf(error_string,"Concentration data error for %s in solution input.", token1);
		error_msg(error_string, CONTINUE);
		return(ERROR);
	}
	if ( (j = copy_token(token, &ptr, &l)) == EMPTY) return(OK);
/*
 *   Read optional data
 */
	strcpy(token1,token);
 ...
 */
		...
		if (copy_token(token, &ptr, &l) != DIGIT) {
			...
			sscanf(token,SCANFORMAT, &(solution[n]->totals[count_mass_balance].gfw));
			if ( (j = copy_token(token, &ptr, &l)) == EMPTY) return(OK);
		}
	}
/*
 ...
 */
	if  ( strcmp_nocase_arg1(token,"pe") == 0 ) {
		solution[n]->totals[count_mass_balance].n_pe = pe_data_store (&(solution[n]->pe), token);
		if ( (j = copy_token(token, &ptr, &l)) == EMPTY) return(OK);
 ...
 *   Must have phase
 */
	solution[n]->totals[count_mass_balance].equation_name = string_hsave(token);
	if ( (j = copy_token(token, &ptr, &l)) == EMPTY) return(OK);
/*
 *   Check for saturation index
 */
	j=sscanf(token,SCANFORMAT, &(solution[n]->totals[count_mass_balance].phase_si));
	if (j != 1 ) {
 ...
 */
			...
			if (i < 1) {
				sprintf(error_string, "Expecting activity-coefficient parameter, a.");
				warning_msg(error_string);
			} 
			opt_save = OPTION_DEFAULT;
			break;
			...
			if (i <= 0) {
				input_error++;
				sprintf(error_string, "Expected the constant to add for log_K definition.");
				error_msg(error_string, CONTINUE);
				break;
			}
			/* set name */
 ...
 */
			for (;;) {
				...
				if (i == EMPTY) {
					error_msg("Expected a solution number with which to equilibrate exchanger.", CONTINUE);
					error_msg(line_save, CONTINUE);
					input_error++;
					break;
				}
 ...
 */
				...
				if( j == UPPER || j == LOWER) {
					...
					if (copy_token(token1, &ptr, &l) != DIGIT) {
                	                        error_msg("Expected a coefficient to relate exchange to kinetic reaction.\n", CONTINUE);
						input_error++;
						break;
        		                }             
 ...
 */
				...
				j = copy_token(token1, &ptr, &l);
					...
					if (token1[0] == 'K' || token1[0] == 'k') {
						exchange[n].comps[count_comps].rate_name = exchange[n].comps[count_comps].phase_name;
						exchange[n].comps[count_comps].phase_name = NULL;
						exchange[n].related_rate = TRUE;
					} else if (token1[0] == 'E' || token1[0] == 'e') {
						exchange[n].related_phases = TRUE;
					} else {
						error_msg("Character string expected to be 'equilibrium_phase' or 'kinetics' to relate exchange to mineral or kinetic reaction.\n", CONTINUE);
						input_error++;
						break;
					}			       
					j = copy_token(token1, &ptr, &l);
                                ...
                                if (j != DIGIT) {
                                        error_msg("Expected a coefficient to relate exchanger to mineral or kinetic reaction.\n", CONTINUE);
					input_error++;
					break;
                                }
				sscanf(token1, SCANFORMAT, &exchange[n].comps[count_comps].phase_proportion);
				...
				error_msg("Expected concentration of exchanger, mineral name, or kinetic reaction name.", CONTINUE);
				error_msg(line_save, CONTINUE);
				input_error++;
				break;
			}
 ...
 */
			for (;;) {
				...
				if (i == EMPTY) {
					error_msg("Expected a solution number with which to equilibrate gas phase.", CONTINUE);
					error_msg(line_save, CONTINUE);
					input_error++;
					break;
				}
 ...
 */
			gas_phase[n].comps = (struct gas_comp *) PHRQ_realloc(gas_phase[n].comps, (size_t) (count_comps + 1) * sizeof(struct gas_comp));
			if (gas_phase[n].comps == NULL) malloc_error();
 ...
 */
			...
			j=sscanf( token,SCANFORMAT, &(gas_phase[n].comps[count_comps - 1].p_read) );
			if (j != 1 ) {
 ...
 */
	...
	for (;;) {
		...
		switch (opt) {
		    ...
		    case 0:                 /* solutions */
		    case 10:                /* solution */
			    inverse[n].solns = read_list_ints(&next_char, &inverse[n].count_solns, TRUE);
			    opt_save = OPTION_ERROR;
			    break;
		    case 1:                 /* uncertainty */
 ...
 */
	if (inverse[n].count_isotopes > 0) {
		qsort (inverse[n].isotopes,
	       ...
	       (size_t) sizeof(struct inv_isotope),
	       inverse_isotope_compare);
	}
...
/* ---------------------------------------------------------------------- */
int read_inv_balances (struct inverse *inverse_ptr, char *ptr)
/* ---------------------------------------------------------------------- */
{
	int j, l, count;
	char token[MAX_LENGTH];
 ...
 */		
		inverse_ptr->elts[inverse_ptr->count_elts].uncertainties = read_list_doubles(&ptr, &count);
 ...
 */		
	inverse_ptr->i_u[inverse_ptr->count_i_u].uncertainties = read_list_doubles(&ptr1, &count);
	inverse_ptr->i_u[inverse_ptr->count_i_u].count_uncertainties = count;
	inverse_ptr->count_i_u++;
	return(OK);
}
/* ---------------------------------------------------------------------- */
int read_inv_phases (struct inverse *inverse_ptr, char *ptr)
/* ---------------------------------------------------------------------- */
{
	int j, l;
	int count_isotopes;
 ...
 */
			...
			sscanf(token, SCANFORMAT, &(isotopes[count_isotopes].ratio_uncertainty));
	...
	if (count_isotopes > 0) {
		...
		inverse_ptr->phases[inverse_ptr->count_phases].count_isotopes = count_isotopes;
	} else {
		...
		switch (opt) {
...
#ifdef SKIP
			kinetics_ptr->comps[count_comps].formula = kinetics_ptr->comps[count_comps].rate_name;
#endif
			...
			if (kinetics_comp_ptr == NULL) {
			  ...
			  if (copy_token (token, &next_char, &l) == DIGIT) {
			    ...
			    sprintf(error_string, "Expecting numerical value for moles of reactant.");
			    error_msg(error_string, CONTINUE);
			    input_error++;
			  }
			}
			...
			if (kinetics_comp_ptr == NULL) {
			  ...
			  if (copy_token (token, &next_char, &l) == DIGIT) {
			    ...
			    sprintf(error_string, "Expecting numerical value for initial moles of reactant.");
			    error_msg(error_string, CONTINUE);
			    input_error++;
			  }
			}
			...
			if (kinetics_ptr->count_steps != 1) {
				error_msg("To define equal time increments, only one total time should be defined.", CONTINUE);
				input_error++;
				break;
			}
			do {
...
/* ---------------------------------------------------------------------- */
int read_delta_h_only (char *ptr, LDBLE *delta_h, DELTA_H_UNIT *units)
/* ---------------------------------------------------------------------- */
{
	int j, l, kilo, joul;
	char token[MAX_LENGTH];
 ...
 */
	...
	j=sscanf(ptr,SCANFORMAT SCANFORMAT SCANFORMAT SCANFORMAT SCANFORMAT, &(log_k[0]), &(log_k[1]), &(log_k[2]), &(log_k[3]), 
		 &(log_k[4]));
	if (j < 1) {
		input_error++;
 ...
 *   find next keyword
 */
	while ( (j = check_line("Subroutine Read", FALSE, TRUE, TRUE, FALSE)) != KEYWORD) {
                                		/* empty, eof, keyword, print */
		if ( j == EOF) return(EOF);
 ...
 */
		...
		if (i != 1) {
			input_error++;
			if (elts_ptr != NULL) {
				sprintf(error_string, "Expected alkalinity for master species, %s, in master species input.", elts_ptr->name);
			} else {
				sprintf(error_string, "Expected alkalinity for master species in master species input.");
			}
			error_msg(error_string, CONTINUE);
			continue;
		}
 ...
 */
		...
		if (i == DIGIT) {
			...
			input_error++;
			if (elts_ptr != NULL) {
				sprintf(error_string, "Expected gram formula weight for master species, %s, in master species input.", elts_ptr->name);
			} else {
				sprintf(error_string, "Expected gram formula weight for master species in master species input.");
			}
			error_msg(error_string, CONTINUE);
			continue;
		}
		...
		if ( strchr(master[count_master]->elt->name,'(') == NULL ) {
			...
			if ( strcmp(master[count_master]->elt->name,"E") != 0 ) {
				...
				if (i == DIGIT) {
					...
					input_error++;
					if (elts_ptr != NULL) {
						sprintf(error_string,"Expected gram formula weight for element, %s.", elts_ptr->name);
					} else {
						sprintf(error_string,"Expected gram formula weight for element.");
					}
 ...
 */
		count_comps++;
		if (mix[n].comps == NULL) {
			mix[n].comps = (struct mix_comp *) PHRQ_malloc( (size_t) sizeof (struct mix_comp));
		} else {
 ...
 */
	...
	if (count_comps <= 0) {
		input_error++;
		error_msg("Must define at least one solution number and mixing fraction for MIX input.", CONTINUE);
	}		
 ...
 */
	...
	for (;;) {
		...
		switch (opt) {
		    ...
		    case 5:                 /* deltah */
			if (phase_ptr == NULL) break;
			read_delta_h_only(next_char, &phase_ptr->logk[1], &phase_ptr->original_units);
			opt_save = OPTION_DEFAULT;
			break;
			...
			/* set name */
			phase_ptr->add_logk[phase_ptr->count_add_logk].name = string_hsave("XconstantX");
			/* read coef */
			phase_ptr->count_add_logk++;
 			opt_save = OPTION_DEFAULT;
 			break;
 ...
 */
	if (count_steps != 1) {
		error_msg("To define equal increments, only one reaction increment should be defined.", CONTINUE);
		input_error++;
		return(ERROR);
	}
	do {
 ...
 */
	...
	switch (next_keyword) {
	    ...
	    case 39:
		input_error++;
		error_msg("Expecting keyword solution, equilibrium_phases, exchange, surface, kinetics, gas_phase, or solid_solutions.", CONTINUE);
		error_msg(line_save, CONTINUE);
	...
	for (;;) {
		...
		switch (opt) {
			...
			if ((j = copy_token(token, &next_char, &l)) != EMPTY) {
				...
				sscanf(token, SCANFORMAT, &(solution[n]->isotopes[count_isotopes].ratio_uncertainty));
			} 
			count_isotopes++;
			break;
		    case 10:                        /* water */
 ...
 */
	...
	switch (next_keyword) {
	    ...
	    case 39:
		input_error++;
		error_msg("Expecting keyword solution, mix, kinetics, reaction, reaction_temperature, equilibrium_phases, exchange, surface, gas_phase, or solid_solutions.", CONTINUE);
		error_msg(line_save, CONTINUE);
	...
	for (;;) {
		...
		if (i == DIGIT) {
			...
			if (strstr(token,"-") != NULL) {
				sprintf(error_string, "USE does not accept a range of numbers, %s.", token);
				warning_msg(error_string);
				sprintf(error_string, "Only %s %d will be used in the batch-reaction calculation.", token1, n_user);
				warning_msg(error_string);
				sprintf(error_string, "NOTE--USE is not needed for ADVECTION and TRANSPORT calculations.");
				warning_msg(error_string);
	...
	for (;;) {
		...
		switch (opt) {
			...
			for (;;) {
				...
				if (i == EMPTY) {
					error_msg("Expected a solution number with which to equilibrate surface.", CONTINUE);
					error_msg(line_save, CONTINUE);
					input_error++;
					break;
				}
 ...
 */
			...
			i = copy_token(token, &ptr, &l);
			if (i != UPPER && token[0] != '['  ) {
				error_msg("Expected surface name to begin with a capital letter.", CONTINUE);
				error_msg(line_save, CONTINUE);
				input_error++;
				break;
			}
 ...
 */
				...
				/* read optional 'equilibrium_phases' or 'kinetics' */
					...
					if (token1[0] == 'K' || token1[0] == 'k') {
						...
						error_msg("Character string expected to be 'equilibrium_phase' or 'kinetics' to relate surface to mineral or kinetic reaction.\n", CONTINUE);
						input_error++;
						break;
					}			       
					j = copy_token(token1, &ptr, &l);
 ...
 */
			...
			if (i >= count_charge) {
				...
				if (surface[n].comps[count_comps].phase_name == NULL && surface[n].comps[count_comps].rate_name == NULL) {
 ...
 */
	...
	for (;;) {
		return_value = check_line ("Surface species equation", FALSE, TRUE, TRUE, TRUE);
		if (return_value == EOF || return_value == KEYWORD ) break;
/*
 *   Get "element" name with valence, allocate space, store
 */
 ...
 */
	...
	if (temperature[n].t == NULL) malloc_error();
 ...
 */
		...
		if (j == 1 ) {
			count_t++;
			temperature_ptr->t = (LDBLE *) PHRQ_realloc(temperature_ptr->t, (size_t) count_t * sizeof(LDBLE));
 ...
 */
	if (count_t != 2) {
		error_msg("To define equal increments, exactly two temperatures should be defined.", CONTINUE);
		error_msg(line_save, CONTINUE);
		input_error++;
		return(ERROR);
	}
	...
	for (;;) {
		...
		switch (opt) {
			...
			print_temp = read_list_ints_range (&next_char, &count_print, TRUE, print_temp);
				...
				sscanf(token, "%d", &l);
				print_temp = PHRQ_realloc(print_temp, (size_t) (count_print + 1) * sizeof(int));
				if (print_temp == NULL) malloc_error();
			...
			punch_temp = read_list_ints_range (&next_char, &count_punch, TRUE, punch_temp);
                        ...
                        while (copy_token(token, &next_char, &l) == DIGIT) {
				sscanf(token, "%d", &l);
				punch_temp = PHRQ_realloc(punch_temp, (size_t) (count_punch + 1) * sizeof(int));
				if (punch_temp == NULL) malloc_error();
 ...
 */
	...
	if (found_item == NULL) return(FALSE);
		...
		if (find_option(&(option[1]), &opt, opt_list, count_opt_list, FALSE) == OK) {
			...
			*next_char = opt_ptr;
			if (pr.echo_input == TRUE) {
				if(!reading_database()) output_msg(OUTPUT_MESSAGE, "\t%s\n", line_save);
			}
		} else {
			if(!reading_database()) output_msg(OUTPUT_MESSAGE, "\t%s\n", line_save);
 ...
 */
	...
	for (;;) {
		...
		switch (opt) {
		    ...
		    case 3:                        /* heading*/
			while (copy_token(token, &next_char, &l) != EMPTY) {
				user_graph_headings = PHRQ_realloc(user_graph_headings, (size_t) (user_graph_count_headings + 1) * sizeof(char *));
				if (user_graph_headings == NULL) malloc_error();
...
/*Modifications of read_user_punch to change the chart's appearance */
            ...
            if (j == 4) SetAxisScale(axis, j, 0, get_true_false(next_char, FALSE));
            }
            break;
            case 7:
	    ...
	    break;
            ...
            if (shifts_as_points == TRUE) chart_type = 0;
                else chart_type = 1;
            break;
            case 10: {
            i = copy_token(token, &next_char, &l);
			str_tolower(token);
			if (i == DIGIT) sscanf(token, "%d", &RowOffset);
            i = copy_token(token, &next_char, &l);
			str_tolower(token);
			if (i == DIGIT) sscanf(token, "%d", &ColumnOffset);
            }
            break;
            case 11:
	    ...
	    /* End of modifications */
			...
			if (user_graph->commands == NULL) malloc_error();
			...
			line_length = strlen(line);
			user_graph->commands = PHRQ_realloc(user_graph->commands, (size_t) (length + line_length + 2) * sizeof(char));
			if (user_graph->commands == NULL) malloc_error();
 ...
 */
			...
			ptr = next_char;
			copy_token(token, &ptr, &l);
			s_s_assemblage[n].s_s[number_s_s].comps[count_comps].name = string_hsave(token);
			/*
			...
			if ( (j = copy_token(token, &ptr, &l)) == EMPTY) {
				...
				j=sscanf( token,SCANFORMAT, &dummy);
				s_s_assemblage[n].s_s[number_s_s].comps[count_comps].moles = (LDBLE) dummy;
				if (j != 1 ) {
			...
			for (i = 0; i < 4; i++) {
				if (copy_token(token, &ptr, &l) != EMPTY) {
					j += sscanf( token,SCANFORMAT, &(s_s_assemblage[n].s_s[number_s_s].p[i]) );
				}
			}
			if (j != 4) {
				sprintf(error_string, "Expected 4 parameters to calculate a0 and a1 from two activity coefficients, assemblage %d, solid solution %s", s_s_assemblage[n].n_user, s_s_assemblage[n].s_s[number_s_s].name);
				error_msg(error_string, CONTINUE);
				input_error++;
			}
			...
			if (j != 4) {
				sprintf(error_string, "Expected 4 parameters to calculate a0 and a1 from two distribution coefficients, assemblage %d, solid solution %s", s_s_assemblage[n].n_user, s_s_assemblage[n].s_s[number_s_s].name);
				error_msg(error_string, CONTINUE);
				input_error++;
			}
			...
			if (j != 2) {
				sprintf(error_string, "Expected 2 miscibility gap fractions of component 2 to calculate a0 and a1, assemblage %d, solid solution %s", s_s_assemblage[n].n_user, s_s_assemblage[n].s_s[number_s_s].name);
				error_msg(error_string, CONTINUE);
				input_error++;
			}
			...
			if (j != 2) {
				sprintf(error_string, "Expected 2 spinodal gap fractions of component 2 to calculate a0 and a1, assemblage %d, solid solution %s", s_s_assemblage[n].n_user, s_s_assemblage[n].s_s[number_s_s].name);
				error_msg(error_string, CONTINUE);
				input_error++;
			}
			...
			if (j != 2) {
				sprintf(error_string, "Expected fraction of component 2 and critical temperature to calculate a0 and a1, assemblage %d, solid solution %s", s_s_assemblage[n].n_user, s_s_assemblage[n].s_s[number_s_s].name);
				error_msg(error_string, CONTINUE);
				input_error++;
			}
			...
			if (j != 2) {
				sprintf(error_string, "Expected fraction of component 2 and sigma pi at alyotropic point to calculate a0 and a1, assemblage %d, solid solution %s", s_s_assemblage[n].n_user, s_s_assemblage[n].s_s[number_s_s].name);
				error_msg(error_string, CONTINUE);
				input_error++;
			}
			...
			j = 0;
			if (copy_token(token, &ptr, &l) != EMPTY) {
				j = sscanf( token,SCANFORMAT, &(s_s_assemblage[n].s_s[number_s_s].tk) );
			}
			if (j != 1) {
				sprintf(error_string, "Expected temperature (Kelvin) for parameters, assemblage %d, solid solution %s, using 298.15 K", s_s_assemblage[n].n_user, s_s_assemblage[n].s_s[number_s_s].name);
				warning_msg(error_string);
			...
			if (j != 1) {
				sprintf(error_string, "Expected temperature (Celcius) for parameters, assemblage %d, solid solution %s, using 25 C", s_s_assemblage[n].n_user, s_s_assemblage[n].s_s[number_s_s].name);
				warning_msg(error_string);
			...
			if (j != 2) {
				sprintf(error_string, "Expected Wg2 and Wg1 Thompson-Waldbaum parameters to calculate a0 and a1, assemblage %d, solid solution %s", s_s_assemblage[n].n_user, s_s_assemblage[n].s_s[number_s_s].name);
				error_msg(error_string, CONTINUE);
				input_error++;
			}
			...
			if (j != 2) {
				sprintf(error_string, "Expected alpha2 and alpha3 Margules parameters to calculate a0 and a1, assemblage %d, solid solution %s", s_s_assemblage[n].n_user, s_s_assemblage[n].s_s[number_s_s].name);
				error_msg(error_string, CONTINUE);
				input_error++;
			}
 ...
 */
			...
			s_s_assemblage[n].s_s = (struct s_s *) PHRQ_realloc(s_s_assemblage[n].s_s, (size_t) (count_s_s + 1) * sizeof(struct s_s));
	...
	for (i = 0; i < s_s_assemblage[n].count_s_s; i++) {
		if (s_s_assemblage[n].s_s[i].p[0] != 0.0 ||
			...
			if (s_s_assemblage[n].s_s[number_s_s].count_comps != 2) {
				sprintf(error_string, "Solid solution, %s, is nonideal. Must define exactly two components (-comp1 and -comp2).", s_s_assemblage[n].s_s[number_s_s].name);
				error_msg(error_string, CONTINUE);
				input_error++;
			}
		}
 ...
 */
	...
	for (;;) {
		...
		switch (opt) {
		    ...
		    case OPTION_ERROR:
			input_error++;
			error_msg("Unknown input in LLNL_AQUEOUS_MODEL_PARAMETERS keyword.", CONTINUE);
			error_msg(line_save, CONTINUE);
			break;
 ...
 */
		    ...
		    case 2:                                   /* temp */
			    count_alloc = 1;
			    llnl_count_temp = 0;
			    i = read_lines_doubles(next_char, &(llnl_temp), &(llnl_count_temp), &(count_alloc), opt_list, count_opt_list, &opt);
		      /*
			    ptr = next_char;
		    ...
		    case 5:                                   /* dh_a */
			    count_alloc = 1;
			    llnl_count_adh = 0;
			    i = read_lines_doubles(next_char, &(llnl_adh), &(llnl_count_adh), &(count_alloc), opt_list, count_opt_list, &opt);
		      /*
			    ptr = next_char;
		    ...
		    case 8:                                   /* dh_b */
			    count_alloc = 1;
			    llnl_count_bdh = 0;
			    i = read_lines_doubles(next_char, &(llnl_bdh), &(llnl_count_bdh), &(count_alloc), opt_list, count_opt_list, &opt);
			    /*
			    ptr = next_char;
		    ...
		    case 10:                                  /* b_dot */
			    count_alloc = 1;
			    llnl_count_bdot = 0;
			    i = read_lines_doubles(next_char, &(llnl_bdot), &(llnl_count_bdot), &(count_alloc), opt_list, count_opt_list, &opt);
			    /*
			    ptr = next_char;
		    ...
		    case 12:                                   /* co2_coefs */
			    ...
			    i = read_lines_doubles(next_char, &(llnl_co2_coefs), &(llnl_count_co2_coefs), &(count_alloc), opt_list, count_opt_list, &opt);
			    /*
			    ptr = next_char;
	...
	if ((llnl_count_temp <= 0) ||
		...
		error_msg("Must define equal number (>0) of temperatures, dh_a, dh_b, and bdot parameters\nin LLNL_AQUEOUS_MODEL", CONTINUE);
		input_error++;
	}
	if (llnl_count_co2_coefs != 5) {
		error_msg("Must define 5 CO2 activity coefficient parameters in LLNL_AQUEOUS_MODEL", CONTINUE);
		input_error++;
	}
	for (i = 1; i < llnl_count_temp; i++) {
		if (llnl_temp[i-1] > llnl_temp[i]) {
			error_msg("Temperatures must be in ascending order in LLNL_AQUEOUS_MODEL", CONTINUE);
			input_error++;
		}
	}
...
/* ---------------------------------------------------------------------- */
int read_lines_doubles(char *next_char, LDBLE **d, int *count_d, int *count_alloc, const char **opt_list, int count_opt_list, int *opt)
/* ---------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------- */
int read_line_doubles(char *next_char, LDBLE **d, int *count_d, int *count_alloc)
/* ---------------------------------------------------------------------- */
{
	int i, j, l, n;
	LDBLE value;
 ...
 */
	...
	for (;;) {
		...
		switch (opt) {
			...
			if (empty == FALSE) {
				sprintf(error_string, "Analytical expression previously defined for %s in NAMED_EXPRESSIONS\nAnalytical expression will be overwritten.", logk_ptr->name);
				warning_msg(error_string);
			}
			read_analytical_expression_only(next_char, &(logk_ptr->log_k[2]));
			for (i = 2; i < 7; i++) {
 ...
 */
	...
	if (i == DIGIT) {
		...
		if (strstr(token,"-") != NULL) {
			error_msg("COPY does not accept a range of numbers for source index", CONTINUE);
			error_msg(line_save, CONTINUE);
			input_error++;
			return(ERROR);
		} 
