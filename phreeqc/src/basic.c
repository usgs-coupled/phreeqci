...
Static Char *numtostr(Char *Result, LDBLE n)
{
  /*string255 s;*/
  char *s;
  long i;
...
Local LDBLE realfactor(struct LOC_exec *LINK)
{
  valrec n;
...
Local LDBLE realexpr(struct LOC_exec *LINK)
{
  valrec n;
...
Local void require(int k, struct LOC_exec *LINK)
{
...
#ifdef SKIP
Local long inot(long i, struct LOC_exec *LINK)
{
  return (-i - 1);
}

Local long ixor(long a, long b, struct LOC_exec *LINK)
{
  return ((a & (~b)) | ((~a) & b));
}
#endif
...
Local valrec factor(struct LOC_exec *LINK)
{
  ...
  if (STR1 == NULL) malloc_error();
  STR2 = (char *) PHRQ_calloc(max_line, sizeof(char));
  if (STR2 == NULL) malloc_error();
...
#ifdef SKIP
/* LINK not used */
Local linerec *findline(long n, struct LOC_exec *LINK)
#endif
...
#ifdef SKIP
Local linerec *mustfindline(long n, struct LOC_exec *LINK)
#endif
...
Local void cmdpunch(struct LOC_exec *LINK)
{
#ifdef SKIP
  boolean semiflag;
#endif
...
Local boolean skiploop(int up, int dn, struct LOC_exec *LINK)
{
  boolean Result;
  long i;
  linerec *saveline;
