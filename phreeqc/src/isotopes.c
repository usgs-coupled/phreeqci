 ...
 */
	...
	for (;;) {
		...
		switch (opt) {
		    ...
		    case 0:                 /* isotope */
			if (elt_ptr == NULL) {
				sprintf(error_string, "The element of which this isotope is a minor isotope has not been defined, %s. ISOTOPES data block.", line);
			...
			if (copy_token(token, &next_char, &l) == EMPTY) {
				sprintf(error_string, "Expecting units for isotopic values, %s. ISOTOPES data block.", line);
				error_msg(error_string, CONTINUE);
				input_error++;
				break;
			}				
			 ...
			 */
			if (copy_token(token, &next_char, &l) == EMPTY) {
				sprintf(error_string, "Expecting isotope ratio of standard, %s. ISOTOPES data block.", line);
				error_msg(error_string, CONTINUE);
				input_error++;
				break;
			}				
			...
			master_isotope_ptr_major->total_is_major = get_true_false(next_char, TRUE);
#endif
			sprintf(error_string, "Obsolete identifier. The total of the element must be the sum of all isotopes. ISOTOPES data block.\n%s", line);
 ...
 */
			if (copy_token(token, &next_char, &l) == EMPTY) {
				sprintf(error_string, "Expecting an element name for isotope definition, %s. ISOTOPES data block.", line);
				error_msg(error_string, CONTINUE);
				input_error++;
				break;
			}				
 ...
 */
			if (copy_token(token, &next_char, &l) == EMPTY) {
				sprintf(error_string, "Expecting a name for calculate_value definition, %s. CALCULATE_VALUES data block.", line);
				error_msg(error_string, CONTINUE);
				input_error++;
				break;
			}				
 ...
 */
			if (copy_token(token, &next_char, &l) == EMPTY) {
				sprintf(error_string, "Expecting a name for isotope_ratio definition, %s. ISOTOPE_RATIOS data block.", line);
				error_msg(error_string, CONTINUE);
				input_error++;
				break;
			}				
			 ...
			 */
			if (copy_token(token, &next_char, &l) == EMPTY) {
				sprintf(error_string, "Expecting a name of isotope for an isotope_ratio definition, %s. ISOTOPE_RATIOS data block.", line);
				error_msg(error_string, CONTINUE);
				input_error++;
				break;
			}				
 ...
 */
			if (copy_token(token, &next_char, &l) == EMPTY) {
				sprintf(error_string, "Expecting a name for isotope_alpha definition, %s. ISOTOPE_ALPHAS data block.", line);
				error_msg(error_string, CONTINUE);
				input_error++;
				break;
			}				
...
/* ---------------------------------------------------------------------- */
{
	...
	if (master_isotope_ptr != NULL) {
	  ...
	  calculate_isotope_moles(master_isotope_ptr->elt, solution_ptr, total_moles);
	} 
	...
	for (i = 0; solution_ptr->totals[i].description != NULL; i++ ) {
		...
		if (master_isotope_ptr == NULL) continue;
	...
	for (i= 0; i < count_master_isotope; i++) {
		if (master_isotope[i]->minor_isotope == TRUE && master_isotope[i]->moles > 0) {
...
/* ---------------------------------------------------------------------- */
{
	 ...
	 */
	if (total_moles <= 0) {
		sprintf(error_string, "Can not calculate molality of isotopes, molality of element is zero, %s", elt_ptr->name);
		warning_msg(error_string);
		return(ERROR);
	}
	m_major = total_moles;
	...
	if ((master_isotope_ptr != NULL) && (master_isotope_ptr->elt == elt_ptr)) {
		memcpy(&(list[count_isotopes]), master_isotope_ptr, sizeof(struct master_isotope));
	...
	for (i = 0; solution_ptr->totals[i].description != NULL; i++ ) {
		...
		master_isotope_ptr = master_isotope_search(ptr);
		if (master_isotope_ptr == NULL) continue;
		if (master_isotope_ptr->elt != elt_ptr) continue;
		memcpy(&(list[count_isotopes]), master_isotope_ptr, sizeof(struct master_isotope));
		if (list[count_isotopes].minor_isotope == FALSE) {
			total_is_major = list[count_isotopes].total_is_major;
		}
	...
	for (iter = 0; iter < itmax; iter++) {
		...
		for (i = 0; i < count_isotopes; i++) {
			...
			sprintf(error_string, "Isotope units not recognized, %s",list[i].units);
			input_error++;
			error_msg(error_string, CONTINUE);
		}
		if (total_is_major == TRUE) break;
	...
	for (j = 0; j < count_master_isotope; j++) {
		for (i = 0; i < count_isotopes; i++) {
			if (list[i].name== master_isotope[j]->name) {
				memcpy(master_isotope[j], &(list[i]), sizeof(struct master_isotope));
			}
		}
	}
	/*
	 ...
	 *  make elt list
	 */
	iso_elt_list = PHRQ_malloc((size_t) (count_isotopes + 1)*sizeof(struct elt_list));
	if (iso_elt_list == NULL) malloc_error();
	for (i = 0; i < count_isotopes; i++) {
		master_ptr = master_bsearch(list[i].name);
		if (master_ptr == NULL) {
			sprintf(error_string, "Did not find element in CALCULATE_ISOTOPES, %s.", list[i].name);
...
/* ---------------------------------------------------------------------- */
{
	...
	r = (master_isotope_ptr->ratio/1000. + 1.0)*master_isotope_ptr->standard;
	master_isotope_ptr->moles = major_total * r;
	return(OK);
}
/* ---------------------------------------------------------------------- */
int from_pct(struct master_isotope *master_isotope_ptr, LDBLE total_moles)
/* ---------------------------------------------------------------------- */
{
	master_isotope_ptr->moles = master_isotope_ptr->ratio/100*master_isotope_ptr->standard*total_moles;
	return(OK);
}
/* ---------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------- */
{
	master_isotope_ptr->moles = master_isotope_ptr->ratio*master_isotope_ptr->standard*mass_water_aq_x/gfw_water;
	return(OK);
}
/* ---------------------------------------------------------------------- */
 ...
 */
	...
	if (pr.initial_isotopes == FALSE || pr.all == FALSE) return(OK);
	if (state != INITIAL_SOLUTION) return(OK);
	if (initial_solution_isotopes == FALSE) return(OK);
/*
 *   Print heading
 */
	...
	for (i = 0; i < count_master_isotope; i++) {
		if (master_isotope[i]->minor_isotope == FALSE) {
			...
			if (print_isotope == FALSE) continue;
			/*
			 *  Print isotope values
			 */
			output_msg(OUTPUT_MESSAGE,"%10s\t%12.5e\t%12.5e\n", master_isotope[i]->name, (double) (master_isotope[i]->moles/mass_water_aq_x), (double) master_isotope[i]->moles); 
			for (j = 0; j < count_master_isotope; j++) {
				if (i == j) continue;
				if ((master_isotope[j]->elt == master_isotope[i]->elt) &&
				    (master_isotope[j]->minor_isotope == TRUE)) {
					output_msg(OUTPUT_MESSAGE,"%10s\t%12.5e\t%12.5e\t%12.5e\t%12s\n", master_isotope[j]->name, (double) (master_isotope[j]->moles/mass_water_aq_x), (double) master_isotope[j]->moles, (double) master_isotope[j]->ratio, master_isotope[j]->units);
				}
			}
 ...
 */
	...
	if (punch.in == FALSE || punch.isotopes == FALSE) return(OK);
	if (punch.count_isotopes == 0) return(OK);
	for (i = 0; i < punch.count_isotopes; i++) {
		...
		if (state == INITIAL_SOLUTION) {
			...
			if (isotope_ratio_ptr != NULL) {
				master_isotope_ptr = master_isotope_search(isotope_ratio_ptr->isotope_name);
				if (master_isotope_ptr != NULL && master_isotope_ptr->minor_isotope == TRUE) {
 ...
 */
	...
	if (punch.in == FALSE || punch.calculate_values == FALSE) return(OK);
	if (punch.count_calculate_values == 0) return(OK);
	for (i = 0; i < punch.count_calculate_values; i++) {
		result = MISSING;
		calculate_value_ptr = calculate_value_search(punch.calculate_values[i].name);
		if (calculate_value_ptr != NULL) {
			result = calculate_value_ptr->value;
		}
		if (punch.high_precision == FALSE) {
			fpunchf(sformatf("V_%s",punch.calculate_values[i].name),"%12.4e\t", result);
		} else {
			fpunchf(sformatf("V_%s",punch.calculate_values[i].name),"%20.12e\t", result);
		}
	}	
	return(OK);
}
 ...
 */
	...
	if (pr.isotope_ratios == FALSE || pr.all == FALSE) return(OK);
	if (state == INITIAL_SOLUTION) return(OK);
/*
 *   Print heading
 */
	...
	if (print_isotope == FALSE) return(OK);
	...
	for (j = 0; j < count_isotope_ratio; j++) {
		if (isotope_ratio[j]->ratio == MISSING) continue;
		...
		output_msg(OUTPUT_MESSAGE,"     %-20s\t%12.5e\t%15.5g  %-10s\n", token, (double) isotope_ratio[j]->ratio, (double) isotope_ratio[j]->converted_ratio, master_isotope_ptr->units);
	}
	output_msg(OUTPUT_MESSAGE,"\n");
	return(OK);
}
 ...
 */
	...
	if (pr.isotope_alphas == FALSE || pr.all == FALSE) return(OK);
	if (state == INITIAL_SOLUTION) return(OK);
/*
 *   Print heading
 */
	...
	output_msg(OUTPUT_MESSAGE,"%-37s%14s%14s%12.1f C\n\n", "     Isotope Ratio", "Solution alpha", "Solution", (double) tc_x);

	for (j = 0; j < count_isotope_alpha; j++) {
		if (isotope_alpha[j]->value == MISSING) continue;
		...
		if (isotope_alpha[j]->named_logk != NULL) {
			...
			output_msg(OUTPUT_MESSAGE,"%-37s%14.5g%14.5g%14.5g\n", token, (double) isotope_alpha[j]->value, (double) log_alpha, (double) (1000*calc_logk_n(isotope_alpha[j]->named_logk)*LOG_10));
		} else {
			output_msg(OUTPUT_MESSAGE,"%-37s%14.5g%14.5g\n", token, (double) isotope_alpha[j]->value, (double) (1000*log(isotope_alpha[j]->value)));
		}
...
/* ---------------------------------------------------------------------- */
{
	...
	for (j = 0; j < count_calculate_value; j++) {
		...
		if (calculate_value_ptr->new_def == TRUE) {
			if (basic_compile(calculate_value[j]->commands, &calculate_value[j]->linebase, &calculate_value[j]->varbase, &calculate_value[j]->loopbase) != 0) {
				sprintf (error_string, "Fatal Basic error in CALCULATE_VALUES %s.", calculate_value[j]->name);
				error_msg(error_string, STOP);
			}
			calculate_value_ptr->new_def = FALSE;
		} 
		if (basic_run(command,  calculate_value[j]->linebase, calculate_value[j]->varbase, calculate_value[j]->loopbase) != 0) {
			sprintf (error_string, "Fatal Basic error in calculate_value %s.", calculate_value[j]->name);
			error_msg(error_string, STOP);
		}				
		if (rate_moles == NAN) {
			sprintf(error_string, "Calculated value not SAVE'd for %s.", calculate_value[j]->name);
			error_msg(error_string, STOP);
		} else {
	...
	for (j = 0; j < count_isotope_ratio; j++) {
		...
		if (calculate_value_ptr->value == MISSING) {
			...
			isotope_ratio_ptr->converted_ratio = convert_isotope(master_isotope_ptr, calculate_value_ptr->value);
		}
	}
	for (j = 0; j < count_isotope_alpha; j++) {
		isotope_alpha_ptr = isotope_alpha[j];
...
/* ---------------------------------------------------------------------- */
{
	...
	sprintf(error_string, "Did not recognize isotope units in convert_isotope, %s", units);
	error_msg(error_string, STOP);
	return (-99.0);
}
/*
...
{
	struct master_isotope *master_isotope_ptr;
	master_isotope_ptr=(struct master_isotope *) PHRQ_malloc(sizeof(struct master_isotope));
	if (master_isotope_ptr == NULL) malloc_error();
/*
...
{
	struct calculate_value *calculate_value_ptr;
	calculate_value_ptr=(struct calculate_value *) PHRQ_malloc(sizeof(struct calculate_value));
	if (calculate_value_ptr == NULL) malloc_error();
/*
 *   set pointers in structure to NULL, variables to zero
 */
	calculate_value_init(calculate_value_ptr);
...
{
	struct isotope_ratio *isotope_ratio_ptr;
	isotope_ratio_ptr=(struct isotope_ratio *) PHRQ_malloc(sizeof(struct isotope_ratio));
	if (isotope_ratio_ptr == NULL) malloc_error();
/*
 *   set pointers in structure to NULL, variables to zero
 */
	isotope_ratio_init(isotope_ratio_ptr);
...
{
	struct isotope_alpha *isotope_alpha_ptr;
	isotope_alpha_ptr=(struct isotope_alpha *) PHRQ_malloc(sizeof(struct isotope_alpha));
	if (isotope_alpha_ptr == NULL) malloc_error();
/*
 *   set pointers in structure to NULL, variables to zero
 */
	isotope_alpha_init(isotope_alpha_ptr);
