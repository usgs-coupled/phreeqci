...
/* ---------------------------------------------------------------------- */
int add_solution (struct solution *solution_ptr, LDBLE extensive, LDBLE intensive)
/* ---------------------------------------------------------------------- */
{
/*
		...
		if (surface_ptr->diffuse_layer == TRUE && surface_ptr->new_def == FALSE) {
			for (j = 0; surface_ptr->charge[i].diffuse_layer_totals[j].elt != NULL; j++) {
				...
				if (master_ptr->s == s_hplus) {
					...
					master_ptr->total += surface_ptr->charge[i].diffuse_layer_totals[j].coef;
				}
			}
		}		
	}
 ...
 */
	...
	if (mix_ptr == NULL) return(OK);
	if (mix_ptr->count_comps <= 0) return(OK);
	sum_fractions = 0.0;
	sum_positive = 0.0;
	count_positive = 0;
 ...
 */
	...
	if (check_pp_assemblage(pp_assemblage_ptr) == OK) return(OK);
/*
 ...
 */
	...
	for (j = 0; pp_assemblage_ptr->next_elt[j].elt != NULL; j++) {
		...
		if (master_ptr->s == s_h2o || master_ptr->s == s_hplus) continue;
		if (master_ptr->total > MIN_TOTAL) continue;
		return(FALSE);
	}
	return(TRUE);
}
 ...
 */
	for (i = 0; i < count_elts; i++) {
		if (elt_list[i].elt->master == NULL) {
			sprintf(error_string, "Element or phase not defined in database, %s.", elt_list[i].elt->name);
 ...
 */
	...
	if (temperature_ptr->count_t > 0) {
		...
		if (step_number > -temperature_ptr->count_t) {
			...
			tc_temp = temperature_ptr->t[0] + (temperature_ptr->t[1] - temperature_ptr->t[0]) *
 ...
 */
	...
	if (gas_phase_ptr == NULL) return(OK);
 ...
 *   Add reaction to totals
 */
	if (kinetics_ptr->totals == NULL) return(OK);
	for (i = 0; kinetics_ptr->totals[i].elt != NULL; i++) {
		master_ptr = kinetics_ptr->totals[i].elt->primary;
		if (master_ptr == NULL) {
			input_error++;
			sprintf(error_string, "Element %s in kinetic reaction not found in database.", kinetics_ptr->totals[i].elt->name);
			...
			for (i = 0; i < count_elts; i++) {
				...
				if (master_ptr->s == s_hplus) {
					...
					continue;
 					...
 					if (state != ADVECTION && state != TRANSPORT && state != PHAST) {
						sprintf(error_string, "Element %s is contained in %s (which has 0.0 mass),"
							"\t\nbut is not in solution or other phases.", elt_list[i].elt->name, pure_phase_ptr[j].phase->name);
						warning_msg(error_string);
					}
/*
	...
	for (i = 0; i < s_s_assemblage_ptr->count_s_s; i++) {
		for (j = 0; j < s_s_assemblage_ptr->s_s[i].count_comps; j++) {
			...
			if (s_s_assemblage_ptr->s_s[i].comps[j].moles <= 0.0) {
				add_elt_list(s_s_assemblage_ptr->s_s[i].comps[j].phase->next_elt, 1.0);
				for (l = 0; l < count_elts; l++) {
					...
					if (master_ptr->s == s_hplus) {
						...
						if (state != ADVECTION && state != TRANSPORT && state != PHAST ) {
							sprintf(error_string, "Element %s is contained in solid solution %s (which has 0.0 mass),\nbut is not in solution or other phases.", elt_list[l].elt->name, s_s_assemblage_ptr->s_s[i].comps[j].phase->name);
							warning_msg(error_string);
						}
					}
					/*
 ...
 */
	for (i =0; i < count_master; i++) {
		master_ptr = master[i];
		if (master_ptr->total >= 0.0) continue;
		if (master_ptr->total > -MIN_TOTAL) {
			master_ptr->total = 0;
			continue;
		}
		if (master_ptr->s == s_eminus || master_ptr->s == s_h2o || master_ptr->s == s_hplus || master_ptr->s == s_h3oplus) {
			master_ptr->total = 0;
			continue;
		}
		sprintf(error_string, "Element %s has negative moles in solution, %e. \n\tErroneous mole balance occurs as moles are added to produce zero moles.\n\tUsually caused by KINETICS, REACTION, or diffuse layer calculation.\n\tMay be due to large time steps in early part of KINETICS simulation or negative concentrations in the diffuse layer.", master_ptr->elt->name, (double) master_ptr->total);
		warning_msg(error_string);
		return(MASS_BALANCE);
	}
	return(OK);
