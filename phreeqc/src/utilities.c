...
/* ---------------------------------------------------------------------- */
{
	...
	for ( i=1; rxn_ptr->token[i].s != NULL; i++) {
		...
		if (master_ptr == NULL ) {
			sprintf(error_string, "Non-master species in secondary reaction, %s.", rxn_ptr->token[0].s->name);
 ...
 */
	...
	if (pr.headings == FALSE) return(OK);
#ifdef PHREEQ98
    if ((CreateToC == TRUE) && (AutoLoadOutputFile == TRUE)) {
        if (strstr(ptr,"Reading") == ptr) AddToCEntry((char*)ptr, 1, outputlinenr);
            else if (strstr(ptr,"Beginning") == ptr) AddToCEntry((char*)ptr, 2, outputlinenr);
 ...
 */
	while ( ( (c=*ptr) != '+' ) &&
		...
		if (c == '[') {
			...
			while ((c=*ptr) != ']') {
				if (c == '\0') {
					sprintf(error_string, "No final bracket \"]\" for element name, %s.", string);
					error_msg(error_string, CONTINUE);
					return(ERROR);
				}
				string[i++]=c;
 ...
 */
		...
		while( (isalpha((int) (c=*ptr1)) == FALSE) &&
			...
			if (j >= MAX_LENGTH ) {
				error_msg("The charge on a species has exceeded MAX_LENGTH characters.", CONTINUE);
				return(ERROR);
			}
			ptr1++;
		}
 ...
 */
	...
	while ( ptr != '\0') {
		ptr++;
		if (*ptr == '/' || *ptr == '\0') {
			sprintf(error_string, "End of line or  ""/"" encountered before end of parentheses, %s.", token);
			error_msg(error_string, CONTINUE);
			return(ERROR);
		}
		paren1[p1++]=*ptr;
	...
	if (strcmp(paren1,paren2) < 0) {
		...
		sprintf(error_string, "Both parts of redox couple are the same, %s.",token);
		error_msg(error_string, CONTINUE);
		return(ERROR);
	}
	return(OK);
...
/*	char all[MAX_LENGTH]; */
#ifdef PHREEQ98
    if (ProcessMessages) ApplicationProcessMessages();
    if (stop_calculations == TRUE) error_msg("Execution canceled by user.", STOP);
#endif

	if (pr.status == FALSE || phast == TRUE) return(OK);
...
/*    Count = DIV(i,SegmentSize); */
    ...
    */
    ...
    {
	Table->Directory[i] = (Segment*)PHRQ_calloc(sizeof(Segment),SegmentSize);
...
void hdestroy_multi(HashTable *Table)
{
    int		i,j;
    Segment	*seg;
    Element	*p,*q;
...
/*    SegmentDir = DIV(h,SegmentSize); */
    ...
    */
	...
	||
	(q = (Element*)PHRQ_calloc(sizeof(Element),1))
	==
...
/*    if (++Table->KeyCount / MUL(Table->SegmentCount,SegmentSize) > Table->MaxLoadFactor) */
    if (++Table->KeyCount / ((Table->SegmentCount) << (SegmentSizeShift)) > Table->MaxLoadFactor)
...
/*	NewSegmentDir = DIV(NewAddress,SegmentSize); */
	...
	if (NewSegmentIndex == 0)
	    Table->Directory[NewSegmentDir] = (Segment*)PHRQ_calloc(sizeof(Segment),SegmentSize);
