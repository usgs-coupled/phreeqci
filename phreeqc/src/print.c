	...
	if (rcsid == NULL) fprintf(stderr," ");
        ...
        for (i=0; i < row_count; i++) {
                ...
                for (j=0; j < column_count; j++) {
                        ...
                        output_msg(OUTPUT_MESSAGE,"%11.2e", (double) array_l[i*max_column_count + j]);
                        k++;
                }
                if (k != 0) {
                        output_msg(OUTPUT_MESSAGE,"\n");
...
/* ---------------------------------------------------------------------- */
{
        ...
        if (pr.hdf == FALSE && (punch.in == FALSE || pr.punch == FALSE)) return(OK); 
/*
 *   Punch results
 */
        ...
        if (pr.user_graph == TRUE) punch_user_graph();
...
/* ---------------------------------------------------------------------- */
int print_diffuse_layer(struct surface_charge *surface_charge_ptr1)
/* ---------------------------------------------------------------------- */
{
/*
 ...
 */     
        ...
        if (j >= count_unknowns) {
                sprintf(error_string, "In print_diffuse_layer: component not found, %s.", surface_charge_ptr1->name);
 ...
 */
        ...
        if (pr.eh == FALSE || pr.all == FALSE) return(OK);

        tk_x=tc_x+273.15;

        first = TRUE;
        for (i=0; i < count_master; i++) {
                if (master[i]->in != TRUE ) continue;
                if (master[i]->primary == TRUE) continue;
/*
 *   Secondary master species has mass balance equation
 */
                master_ptr0 = master[i]->elt->primary;
                for (k = i + 1; k < count_master; k++) {
                        if (master[k]->in != TRUE) continue;
                        master_ptr1 = master[k]->elt->primary;
                        if ( master_ptr1 != master_ptr0) break;
/*
 ...
 */
                        ...
                        output_msg(OUTPUT_MESSAGE,"\t%-15s%12.4f%12.4f\n", token, (double) pe, (double) eh);
                }
        }
        if (first == FALSE) output_msg(OUTPUT_MESSAGE,"\n");
        return(OK);
}
/* ---------------------------------------------------------------------- */
 ...
 */
        ...
        if (exchange_ptr == NULL || pr.exchange == FALSE || pr.all == FALSE) return(OK);
 ...
 */
                if (name1 != name) {
                        name = name1;
                        output_msg(OUTPUT_MESSAGE,"%-14s%12.3e mol", name, (double) master_ptr->unknown->moles);
                        ...
                        if (master_ptr->unknown->exch_comp->rate_name != NULL) {
                                output_msg(OUTPUT_MESSAGE, "\t[%g (mol %s)/(mol kinetic reactant %s)]",
                                        (double) master_ptr->unknown->exch_comp->phase_proportion,
                                        master_ptr->unknown->exch_comp->formula,
...
/* !!!!! */
                if (master_ptr->total > 1.0e-10) {
                        ...
                        output_msg(OUTPUT_MESSAGE,"\t%-15s%12.3e%12.3e%12.3e%10.3f\n",
                                ...
                                (double) (species_list[i].s->moles * dum2 * species_list[i].s->equiv),
 ...
 */
        ...
        if (pr.gas_phase == FALSE || pr.all == FALSE) return(OK);
        if (use.gas_phase_ptr == NULL) return(OK);
        if (use.gas_phase_ptr->type == PRESSURE) {
                if (gas_unknown == NULL) return(OK);
                if (gas_unknown->moles < 1e-12) return(OK);
                use.gas_phase_ptr->total_moles = gas_unknown->moles;
                use.gas_phase_ptr->volume = use.gas_phase_ptr->total_moles * R_LITER_ATM * tk_x / use.gas_phase_ptr->total_p;
        }
/*
 *   Print heading
 */
 ...
 */
                if (use.gas_phase_ptr->comps[j].phase->in == TRUE) {
                        ...
                        for (rxn_ptr = use.gas_phase_ptr->comps[j].phase->rxn_x->token + 1; rxn_ptr->s != NULL; rxn_ptr++) {
...
/* ---------------------------------------------------------------------- */
{
	 ...
	 */
        ...
        if (pr.s_s_assemblage == FALSE || pr.all == FALSE) return(OK);
 ...
 */
        ...
        if (pr.use == FALSE || pr.all == FALSE) return(OK);
        if (state < REACTION || use.irrev_in == FALSE) return(OK);
        if (state == TRANSPORT && transport_step == 0) return(OK);
 ...
 */
        ...
        output_msg(OUTPUT_MESSAGE,"\t%11.3e moles of the following reaction have been added:\n\n", (double) step_x);
/*
 *  Print reaction
 */
 ...
 */
        ...
        for (i = 0; irrev_ptr->elts[i].elt != NULL; i++) {
                output_msg(OUTPUT_MESSAGE,"\t%-15s%10.2f\n", irrev_ptr->elts[i].elt->name, (double) irrev_ptr->elts[i].coef);
        }
        output_msg(OUTPUT_MESSAGE,"\n");
        return(OK);
}
 ...
 */
        ...
        if (pr.kinetics == FALSE || pr.all == FALSE) return(OK);
        if (state < REACTION) return(OK);
        kinetics_ptr = NULL;
        ...
        if (kinetics_ptr == NULL) return(OK);
/*
 *   determine time step
 */ 
        if (state == TRANSPORT || state == PHAST) {
        ...
        if (incremental_reactions == TRUE) {
                if (kinetics_ptr->count_steps > 0) {
                        ...
                        if (reaction_step > -kinetics_ptr->count_steps) {
                                ...
                                sim_time = reaction_step * kinetics_ptr->steps[0] / ( (LDBLE) (-kinetics_ptr->count_steps));
                        }
                }
        }
/*
 ...
 */
	if (state == TRANSPORT) {
		...
		output_msg(OUTPUT_MESSAGE, "\tTime:      %g seconds\n",(double) (initial_total_time + advection_step * advection_kin_time));
                output_msg(OUTPUT_MESSAGE,"\tTime step: %g seconds\n\n", (double) kin_time_x);
	} else if (state == PHAST) {
		...
		if (incremental_reactions == FALSE) {
			...
			output_msg(OUTPUT_MESSAGE,"\tTime step: %g seconds  (Incremented time: %g seconds)\n\n", (double) kin_time_x, (double) sim_time);
		}
	}
        ...
        for (i = 0; i < kinetics_ptr->count_comps; i++) {
                if (state != TRANSPORT && state != PHAST) {
                        ...
                        output_msg(OUTPUT_MESSAGE,"\t%-15s%12.3e%12.3e", 
                                kinetics_ptr->comps[i].rate_name, 
                                (double) (kinetics_ptr->comps[i].m - kinetics_ptr->comps[i].initial_moles), 
                                (double) kinetics_ptr->comps[i].m);
                }
 ...
 */
        ...
        if (pr.use == FALSE || pr.all == FALSE) return(OK);
        if (use.mix_in == FALSE || state < REACTION) return(OK);
	if (state == TRANSPORT) {
 ...
 */
	...
	if (state == TRANSPORT) {
		...
		output_msg(OUTPUT_MESSAGE,"Mixture %d.\t%s\n\n", mix_ptr->n_user, mix_ptr->description); 
	}
 ...
 */
	...
	LDBLE la_eminus;
        ...
        if (pr.saturation_indices == FALSE || pr.all == FALSE) return(OK);
	if (state == INITIAL_SOLUTION) {
		iap = 0;
                for (rxn_ptr = pe_x[default_pe_x].rxn->token + 1; rxn_ptr->s != NULL; rxn_ptr++) {
 ...
 */
        ...
        for (i=0; i < count_phases; i++) {
                if (phases[i]->in == FALSE || phases[i]->type != SOLID) continue;
/*
 ...
 */
			...
			/* fprintf(output,"\t%s\t%f\t%e\n", rxn_ptr->s->name, rxn_ptr->s->la, rxn_ptr->coef); */
                ...
                si=-phases[i]->lk + iap;
                output_msg(OUTPUT_MESSAGE,"\t%-15s%7.2f%8.2f%8.2f  %s\n",phases[i]->name, (double) si, (double) iap, (double) phases[i]->lk, phases[i]->formula);
		si1 = si;
#endif
 ...
 */
	...
	LDBLE lm;
        ...
        if (pr.species == FALSE || pr.all == FALSE) return(OK);
	min = -1000;
 ...
 */
                if (name1 != name) {
                        name = name1;
                        output_msg(OUTPUT_MESSAGE,"%-14s%12.3e\n",name, (double) (master_ptr->total/mass_water_aq_x));
 ...
 */
                                if (x[k]->moles >= MIN_RELATED_SURFACE) {
                                        molfrac = (double) (species_list[i].s->moles) / x[k]->moles * species_list[i].s->equiv;
                                } else {
                                        molfrac = 0.0;
                                }
                                output_msg(OUTPUT_MESSAGE,"\t%-15s%12.3e%12.3f%12.3e%12.3f\n",
 ...
 */
        ...
        if (pr.totals == FALSE || pr.all == FALSE) return(OK);
	...
	output_msg(OUTPUT_MESSAGE,"\t%-15s%12s%12s\n\n","Elements","Molality","Moles"); 
        for (i = 0; i < count_unknowns; i++) {
                ...
                if (x[i] == ph_unknown) continue;
                if (x[i] == pe_unknown) continue;
 ...
 *   pH
 */
        output_msg(OUTPUT_MESSAGE,"%45s%7.3f    ","pH  = ",(double) (-(s_hplus->la)));
        if (ph_unknown == NULL) {
                output_msg(OUTPUT_MESSAGE,"\n");
 ...
 *    pe
 */
        output_msg(OUTPUT_MESSAGE,"%45s%7.3f    ","pe  = ",(double) (-(s_eminus->la)));
        if (pe_unknown == NULL) {
                output_msg(OUTPUT_MESSAGE,"\n");
 ...
 *   Others
 */
        output_msg(OUTPUT_MESSAGE,"%45s%7.3f\n","Activity of water  = ",exp( s_h2o->la * LOG_10));
        output_msg(OUTPUT_MESSAGE,"%45s%11.3e\n","Ionic strength  = ", (double) mu_x);
        output_msg(OUTPUT_MESSAGE,"%45s%11.3e\n","Mass of water (kg)  = ", (double) mass_water_aq_x);
        if (alkalinity_unknown == NULL) {
                output_msg(OUTPUT_MESSAGE,"%45s%11.3e\n","Total alkalinity (eq/kg)  = ", (double) (total_alkalinity/mass_water_aq_x));
        }
        if (carbon_unknown == NULL) {
                output_msg(OUTPUT_MESSAGE,"%45s%11.3e\n","Total carbon (mol/kg)  = ", (double) (total_carbon/mass_water_aq_x));
        }
        output_msg(OUTPUT_MESSAGE,"%45s%11.3e\n","Total CO2 (mol/kg)  = ", (double) (total_co2/mass_water_aq_x));
        output_msg(OUTPUT_MESSAGE,"%45s%7.3f\n","Temperature (deg C)  = ", (double) tc_x);
 ...
 */
        ...
        if (pr.user_print == FALSE || pr.all == FALSE) return(OK);
        if (user_print->commands == NULL) return(OK);
        kinetics_ptr = NULL;
        if (use.kinetics_in == TRUE) {
        ...
        if (user_print->new_def == TRUE) {
          /*            basic_renumber(user_print->commands, &user_print->linebase, &user_print->varbase, &user_print->loopbase); */
                if (basic_compile(user_print->commands, &user_print->linebase, &user_print->varbase, &user_print->loopbase) != 0) {
                        error_msg("Fatal Basic error in USER_PRINT.", STOP);
                }
                user_print->new_def = FALSE;
        } 
        if (basic_run(command,  user_print->linebase, user_print->varbase, user_print->loopbase) != 0) {
                error_msg("Fatal Basic error in USER_PRINT.", STOP);
        }
        output_msg(OUTPUT_MESSAGE,"\n");
        if (use.kinetics_in == TRUE) {
 ...
 */
        ...
        if (pr.use == FALSE || pr.all == FALSE) return(OK);
        if (state < REACTION || phast == TRUE) return(OK);
/*
 *   Mixture or Solution
 */
        if (use.mix_in == TRUE) {
 ...
 */
        ...
        if (use.s_s_assemblage_in == TRUE) {
                s_s_assemblage_ptr = s_s_assemblage_bsearch(use.n_s_s_assemblage_user, &n);
 ...
 */
        ...
        if (punch.count_gases <= 0) return(OK);
        p = 0.0;
        total_moles = 0.0;
        volume = 0.0;
        ...
        for (i = 0; i < punch.count_gases; i++) {
                ...
                if (use.gas_phase_ptr != NULL && punch.gases[i].phase != NULL) {
                        for( j = 0; j < use.gas_phase_ptr->count_comps; j++) {
                                ...
                                if (use.gas_phase_ptr->comps[j].phase != punch.gases[i].phase) continue;
                                moles = use.gas_phase_ptr->comps[j].phase->moles_x;
                                if ( moles <= MIN_TOTAL ) moles = 0.0;
                                break;
                        }
 ...
 */
		...
		if (use.s_s_assemblage_ptr != NULL) {
			for( j = 0; j < use.s_s_assemblage_ptr->count_s_s; j++) {
				for (i = 0; i < use.s_s_assemblage_ptr->s_s[j].count_comps; i++) {
					if (strcmp_nocase(punch.s_s[k].name, use.s_s_assemblage_ptr->s_s[j].comps[i].name) == 0) {
						if (use.s_s_assemblage_ptr->s_s[j].s_s_in == TRUE) {
 ...
 */
        ...
        for (j = 0; j < punch.count_totals; j++) {
                ...
                if (punch.high_precision == FALSE) {
                        fpunchf(sformatf("%s(mol/kgw)", punch.totals[j].name),"%12.4e\t", molality);
                } else {
                        fpunchf(sformatf("%s(mol/kgw)", punch.totals[j].name),"%20.12e\t", molality);
                }
        }
        return(OK);
}
 ...
 */
        ...
        for (j = 0; j < punch.count_molalities; j++) {
                ...
                if (punch.high_precision == FALSE) {
                        fpunchf(sformatf("m_%s(mol/kgw)", punch.molalities[j].name),"%12.4e\t", molality);
                } else {
                        fpunchf(sformatf("m_%s(mol/kgw)", punch.molalities[j].name),"%20.12e\t", molality);
                }
        }
        return(OK);
}
 ...
 */
        ...
        for (i = 0; i < punch.count_pure_phases; i++) {
                ...
                if (punch.pure_phases[i].phase != NULL) {
                        for( j = 0; j < count_unknowns; j++) {
                                if (x == NULL || x[j]->type != PP) continue;
/*
 *   Print pure phase assemblage data
 */
                                if (punch.pure_phases[i].phase != x[j]->pure_phase->phase) continue;
                                if (state != TRANSPORT && state != PHAST) {
                                        moles = x[j]->moles;
                                        delta_moles = x[j]->moles - x[j]->pure_phase->moles - x[j]->pure_phase->delta;
                                } else {
                                        moles = x[j]->moles;
                ...
                if (punch.high_precision == FALSE) {
                        ...
                        fpunchf(sformatf("d_%s", punch.pure_phases[i].name),"%12.4e\t", delta_moles);
                } else {
                        fpunchf(punch.pure_phases[i].name,"%20.12e\t", moles);
                        fpunchf(sformatf("d_%s", punch.pure_phases[i].name),"%20.12e\t", delta_moles);
                }
        }
        return(OK);
}
 ...
 */
        ...
        if (punch.dist == TRUE) {
                if (state == ADVECTION) {
                        ...
                        fpunchf(PHAST_NULL("dist_x"), gformat, cell_data[cell-1 ].mid_cell_x);
                } else {
                        fpunchf(PHAST_NULL("dist_x"), dformat, -99);
                }
        }
        if (punch.time == TRUE) {
                if (state == REACTION && incremental_reactions == TRUE && use.kinetics_ptr != NULL) {
                        if (use.kinetics_ptr->count_steps > 0) {
                                ...
                                for (i = 0; i < reaction_step; i++) {
                                        if (i < use.kinetics_ptr->count_steps) {
                                                ...
                                                kin_time_x += use.kinetics_ptr->steps[use.kinetics_ptr->count_steps - 1];
                                        }
                                }
                                ...
                                if (reaction_step > -use.kinetics_ptr->count_steps) {
                                        ...
                                        kin_time_x = reaction_step * use.kinetics_ptr->steps[0] / ( (LDBLE) (-use.kinetics_ptr->count_steps));
                                }
                        }
                }
                if (state == REACTION) {
                        ...
                        fpunchf(PHAST_NULL("time"), gformat, initial_total_time + rate_sim_time);
                } else if (state == ADVECTION) {
 ...
 */
        ...
        if (punch.alk == TRUE) {
                if (punch.high_precision == FALSE) {
                        fpunchf("Alk(eq/kgw)","%12g\t", (double) (total_alkalinity/mass_water_aq_x));
                } else {
                        fpunchf("Alk(eq/kgw)","%20.12e\t", (double) (total_alkalinity/mass_water_aq_x));
                }
        }
        if (punch.mu == TRUE) {
                if (punch.high_precision == FALSE) {
 ...
 */
                        iap = 0.0;
                        for (rxn_ptr = punch.si[i].phase->rxn_x->token + 1; rxn_ptr->s != NULL; rxn_ptr++) {
                                iap += rxn_ptr->s->la * rxn_ptr->coef;
                        }
        ...
        for (i = 0; i < punch.count_kinetics; i++) {
                ...
                if (kinetics_ptr != NULL) {
                        for (j = 0; j < kinetics_ptr->count_comps; j++) {
                                if (strcmp_nocase(punch.kinetics[i].name, kinetics_ptr->comps[j].rate_name) == 0) {
                                        if (state != TRANSPORT && state != PHAST) {
                                                moles = kinetics_ptr->comps[j].m;
                                                delta_moles = -kinetics_ptr->comps[j].moles;
                                        } else {
                                                moles = kinetics_ptr->comps[j].m;
                                                delta_moles = kinetics_ptr->comps[j].m - kinetics_ptr->comps[j].initial_moles;
                                        }
                                        break;
                                }
                        }
                ...
                if (punch.high_precision == FALSE) {
                        ...
                        fpunchf(sformatf("dk_%s", punch.kinetics[i].name),"%12.4e\t", -delta_moles);
                } else {
                        fpunchf(sformatf("k_%s", punch.kinetics[i].name),"%20.12e\t", moles);
                        fpunchf(sformatf("dk_%s", punch.kinetics[i].name),"%20.12e\t", -delta_moles);
                }
        }
        return(OK);
}
 ...
 */
        ...
        if (user_punch->commands == NULL) return(OK);
        if (user_punch->new_def == TRUE) {
                if (basic_compile(user_punch->commands, &user_punch->linebase, &user_punch->varbase, &user_punch->loopbase) != 0) {
                        error_msg("Fatal Basic error in USER_PUNCH.", STOP);
                }
                user_punch->new_def = FALSE;
        } 
        if (basic_run(command,  user_punch->linebase, user_punch->varbase, user_punch->loopbase) != 0) {
                error_msg("Fatal Basic error in USER_PUNCH.", STOP);
        }
        return(OK);
}
...
/*      //if (punch.user_punch == FALSE) return(OK); */
/*          //if (punch.in == FALSE) return(OK); */
        if (user_graph->commands == NULL) return(OK);
    if (((state == INITIAL_SOLUTION) || (state == INITIAL_EXCHANGE) || (state == INITIAL_SURFACE) || (state == INITIAL_GAS_PHASE)) && (graph_initial_solutions == FALSE)) return(OK);
    if (FirstCallToUSER_GRAPH) AddSeries = TRUE;
    if (state == REACTION) {
        /*if (reaction_step == 1) AddSeries = TRUE;
            else AddSeries = FALSE; */
        if (reaction_step > 1) AddSeries = FALSE;
    }
    if (state == ADVECTION) {
        if (advection_step == 0 && graph_initial_solutions == FALSE) return(OK);
        if (((chart_type == 1) && (advection_step == punch_ad_modulus)) ||
            ((chart_type == 0) && (advection_step != prev_advection_step))) AddSeries = TRUE;
            else AddSeries = FALSE;
    }
    if (state == TRANSPORT) {
        if (transport_step == 0 && graph_initial_solutions == FALSE) return(OK);
        if (((chart_type == 1) && (transport_step == punch_modulus)) ||
            ((chart_type == 0) && (transport_step != prev_transport_step))) AddSeries = TRUE;
            else AddSeries = FALSE;
    }
        if (user_graph->new_def == TRUE) {
                if (basic_compile(user_graph->commands, &user_graph->linebase, &user_graph->varbase, &user_graph->loopbase) != 0) {
                        error_msg("Fatal Basic error in USER_GRAPH.", STOP);
                }
                user_graph->new_def = FALSE;
        }
        if (basic_run(command,  user_graph->linebase, user_graph->varbase, user_graph->loopbase) != 0) {
                error_msg("Fatal Basic error in USER_GRAPH.", STOP);
        }
    if (state == ADVECTION) prev_advection_step = advection_step;
    if (state == TRANSPORT) prev_transport_step = transport_step;
...
extern void HDFWriteHyperSlabV(const char* name, const char* format, va_list argptr);
#endif

#if defined(USE_MPI) && defined(HDF5_CREATE) && defined(MERGE_FILES)
extern int Merge_fpunchf(const int length, const char* format, va_list argptr);
#endif
...
int fpunchf_user(int user_index, const char* format, ...)
{
	...
	if (user_index < user_punch_count_headings)	{
		...
		if (s_warning == 0) {
			sprintf(error_string, "USER_PUNCH: Headings count doesn't match number of calls to PUNCH.\n");
			warning_msg(error_string);
			s_warning = 1;
		}
		sprintf(buffer, "no_heading_%d", (user_index - user_punch_count_headings) + 1);
		name = buffer;
	}
