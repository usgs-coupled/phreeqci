...
/* ---------------------------------------------------------------------- */
{
	...
	for (k = 0; k < count_s_x; k++) {
		...
		for (i=1; i < count_trxn; i++) {
			...
			if (token_ptr->s->type != SURF) continue;
 ...
 */
	...
	for (j =0; j < count_unknowns; j++) {
		...
		if (surface_name != NULL) {
			if (strcmp(name, surface_name) == 0) break;
		} else {
			break;
		}
	}
	if (j >= count_unknowns) return(0);
	...
	if (strcmp_nocase("psi", total_name) == 0) {
		...
		if (use.surface_ptr->edl == TRUE) {
			...
			if ((x[j]->surface_charge->specific_area * x[j]->surface_charge->grams) > 0) {
				return((LDBLE) (charge * F_C_MOL / (x[j]->surface_charge->specific_area * x[j]->surface_charge->grams)));
			} else {
				return(0);
			}
		} else {
...
/* ---------------------------------------------------------------------- */
{
	...
	if (use.pp_assemblage_in == FALSE || use.pp_assemblage_ptr == NULL) return(0);
	for( j = 0; j < count_unknowns; j++) {
 ...
 */
	if (j == count_unknowns) {
		...
		for (i=0; i < use.pp_assemblage_ptr->count_comps; i++) {
			if (strcmp_nocase(use.pp_assemblage_ptr->pure_phases[i].name,phase_name) == 0) {
...
/* ---------------------------------------------------------------------- */
{
	...
	if (use.s_s_assemblage_in == FALSE || use.s_s_assemblage_ptr == NULL) return(0.0);
...
/* ---------------------------------------------------------------------- */
{
	...
	for (j = 0; j < use.s_s_assemblage_ptr->count_s_s; j++) {
		for (i=0; i < use.s_s_assemblage_ptr->s_s[j].count_comps; i++) {
			if (strcmp_nocase(use.s_s_assemblage_ptr->s_s[j].comps[i].name, s_s_comp_name) == 0) {
 ...
 */
{
	...
	if (name == NULL) {
		sprintf(error_string, "Definition for calculated value not found, %s", name);
		input_error++;
		error_msg(error_string, CONTINUE);
		return (MISSING);
	}
	if (calculate_value_ptr->calculated == FALSE) {
		sprintf(error_string, "Calculated value used before it has been calculated, %s.\nCalculated values are evalutated in the order in which they were defined.", name);
		input_error++;
		error_msg(error_string, CONTINUE);
		return (MISSING);
	}
...
/* ---------------------------------------------------------------------- */
{
	...
	if (use.kinetics_in == FALSE || use.kinetics_ptr == NULL) return(0);
	for (i = 0; i < use.kinetics_ptr->count_comps; i++) {
		if (strcmp_nocase(use.kinetics_ptr->comps[i].rate_name, kinetics_name) == 0) {
			return (use.kinetics_ptr->comps[i].m);
		}
	}

	sprintf(error_string, "No data for rate %s in KINETICS keyword.", kinetics_name);
	...
	for (i=0; i < use.gas_phase_ptr->count_comps; i++) {
		if (match_elts_in_species(use.gas_phase_ptr->comps[i].phase->formula,template) == TRUE) {
			if (name == NULL) {
				...
				for (next_elt = use.gas_phase_ptr->comps[i].phase->next_elt; next_elt->elt != NULL; next_elt++) {
					if (strcmp(next_elt->elt->name, name) == 0) {
						tot += next_elt->coef * use.gas_phase_ptr->comps[i].phase->moles_x;
						break;
					}
				}
			}
		...
		if (strcmp_nocase(use.s_s_assemblage_ptr->s_s[j].name, template) == 0) {
			...
			for (i=0; i < use.s_s_assemblage_ptr->s_s[j].count_comps; i++) {
				if (name == NULL) {
					...
					for (next_elt = use.s_s_assemblage_ptr->s_s[j].comps[i].phase->next_elt; next_elt->elt != NULL; next_elt++) {
						if (strcmp(next_elt->elt->name, name) == 0) {
							tot += next_elt->coef * use.s_s_assemblage_ptr->s_s[j].comps[i].moles;
							break;
						}
					}
				}
...
/* ---------------------------------------------------------------------- */
{
	...
	for (j=0; j < use.s_s_assemblage_ptr->count_s_s; j++) {
		for (i=0; i < use.s_s_assemblage_ptr->s_s[j].count_comps; i++) {
			if (match_elts_in_species(use.s_s_assemblage_ptr->s_s[j].comps[i].phase->formula,template) == TRUE) {
				if (name == NULL) {
					tot += use.s_s_assemblage_ptr->s_s[j].comps[i].moles;
				} else {
					for (next_elt = use.s_s_assemblage_ptr->s_s[j].comps[i].phase->next_elt; next_elt->elt != NULL; next_elt++) {
 ...
 */
	...
	for ( i = 1; i < count_match_tokens; i++) {
		if ((isupper((int) (match_tokens[i].name[0])) != FALSE) && (match_tokens[i].name == match_tokens[i1].name)) {
	...
	switch (case_no) {
	    case 0:
		    /* exact match */
		    if (strcmp(token, template1) == 0) match = TRUE;
		    break;
	    case 1:
		    /* leading wild card */
		    if ((ptr = strstr(token, template1)) == NULL) {
			    ...
			    if (strcmp(ptr, template1) == 0) match = TRUE;
		    }
		    break;
	    case 2:
		    /* trailing wild card */
		    if (strstr(token, template1) == token) match = TRUE;
		    break;
	    case 3:
		    /* trailing wild card */
		    if (strstr(token, template1) != NULL) match = TRUE;
		    break;
	}
	return (match);
}
...
/* ---------------------------------------------------------------------- */
{
	...
	if ((ptr = strstr(*string, "{")) == NULL) return(FALSE);
	strcpy(bracket_string, ptr);
	if ((ptr1 = strstr(bracket_string, "}")) == NULL) {
		sprintf(error_string, "No matching bracket (}) in isotope template string %s", *string);
		error_msg(error_string, CONTINUE);
		input_error++;
		return(FALSE);
	}
 ...
 */
	...
	if (use.surface_ptr == NULL) return(0);
...
/* ---------------------------------------------------------------------- */
{
	...
	t = 0.0;
	if (master_ptr == NULL) {
		sprintf(error_string,"Can not find definition for master species, %s.", total_name);
		warning_msg(error_string);
/*
...
/* ---------------------------------------------------------------------- */
{
	...
	for (i = 0; i < count_master; i++) {
		master_ptr=master[i];
 		if (master_ptr->primary == TRUE && master_ptr->total_primary <= 0) continue;
 		if (master_ptr->in == FALSE && (master_ptr->primary == FALSE || master_ptr->total_primary == 0)) continue;
		/*
		 *  H and O
		 */
		...
		if (master_ptr->primary == TRUE) {
			if (master_ptr->total_primary > 0) {
				...
				t = 0;
				for (j = master_ptr->number + 1; master[j]->elt->primary == master_ptr; j++) {
					t += master[j]->total;
				}
			}
			/*
 ...
 */
	...
	for( i = 0; i < use.gas_phase_ptr->count_comps; i++) {
		sys[count_sys].name = string_duplicate(use.gas_phase_ptr->comps[i].phase->name);
 ...
 */
	...
	for( k = 0; k < use.s_s_assemblage_ptr->count_s_s; k++) {
		for (i = 0; i < use.s_s_assemblage_ptr->s_s[k].count_comps; i++) {
			sys[count_sys].name = string_duplicate(use.s_s_assemblage_ptr->s_s[k].comps[i].phase->name);
 ...
 */
	for (i = 0; i < count_s_x; i++) {
		...
		for (k = 0; k < count_unknowns; k++) {
			if (x[k]->type != SURFACE_CB) continue;
			i++;
			/*
			...
			for (j = 0; j < count_s_x; j++) {
				...
				molality = under(s_x[j]->lm);
				count_g = s_x[j]->diff_layer[i].count_g; 
				moles_excess = mass_water_aq_x * molality * x[k]->surface_charge->g[count_g].g; 
				moles_surface = mass_water_surface * molality + moles_excess;
				/*
				 *   Accumulate elements in diffuse layer
				 */
 ...
 */
	if (use.pp_assemblage_in == TRUE && use.pp_assemblage_ptr != NULL) {
		for( i = 0; i < count_unknowns; i++) {
			if (x[i]->type != PP) continue;
			if (x[i]->pure_phase->add_formula != NULL) continue;
			count_elts = 0;
			paren_count = 0;
		...
		for( k = 0; k < use.s_s_assemblage_ptr->count_s_s; k++) {
			if (use.s_s_assemblage_ptr->s_s[k].s_s_in == TRUE) {
				for (i = 0; i < use.s_s_assemblage_ptr->s_s[k].count_comps; i++) {
					count_elts = 0;
					paren_count = 0;
					add_elt_list(use.s_s_assemblage_ptr->s_s[k].comps[i].phase->next_elt, use.s_s_assemblage_ptr->s_s[k].comps[i].moles);
					if (count_elts > 0 ) {
						qsort (elt_list, (size_t) count_elts,
						       (size_t) sizeof(struct elt_list), elt_list_compare);
						elt_list_combine();
 ...
 */
		...
		for( i = 0; i < use.gas_phase_ptr->count_comps; i++) {
			if (use.gas_phase_ptr->comps[i].phase->in == TRUE) {
				count_elts = 0;
				paren_count = 0;
				add_elt_list(use.gas_phase_ptr->comps[i].phase->next_elt, use.gas_phase_ptr->comps[i].phase->moles_x);
		...
		for( k = 0; k < use.s_s_assemblage_ptr->count_s_s; k++) {
			if (use.s_s_assemblage_ptr->s_s[k].s_s_in == TRUE) {
				for (i = 0; i < use.s_s_assemblage_ptr->s_s[k].count_comps; i++) {
					count_elts = 0;
					paren_count = 0;
					add_elt_list(use.s_s_assemblage_ptr->s_s[k].comps[i].phase->next_sys_total, use.s_s_assemblage_ptr->s_s[k].comps[i].moles);
					if (count_elts > 0 ) {
						qsort (elt_list, (size_t) count_elts,
						       (size_t) sizeof(struct elt_list), elt_list_compare);
						elt_list_combine();
 ...
 */
		...
		for( i = 0; i < use.gas_phase_ptr->count_comps; i++) {
			if (use.gas_phase_ptr->comps[i].phase->in == TRUE) {
				count_elts = 0;
				paren_count = 0;
				add_elt_list(use.gas_phase_ptr->comps[i].phase->next_sys_total, use.gas_phase_ptr->comps[i].phase->moles_x);
	...
	if (s_s_assemblage_ptr != NULL) {
		for( i = 0; i < s_s_assemblage_ptr->count_s_s; i++) {
			for (j = 0; j < s_s_assemblage_ptr->s_s[i].count_comps; j++) {
				add_elt_list(s_s_assemblage_ptr->s_s[i].comps[j].phase->next_elt, s_s_assemblage_ptr->s_s[i].comps[j].moles);
			}
		}
	}
        if (gas_phase_ptr != NULL) {
		for( i = 0; i < gas_phase_ptr->count_comps; i++) {
			add_elt_list(gas_phase_ptr->comps[i].phase->next_elt, gas_phase_ptr->comps[i].moles);
		}
	}
	if (pp_assemblage_ptr != NULL) {
		for( i = 0; i < pp_assemblage_ptr->count_comps; i++) {
			add_elt_list(pp_assemblage_ptr->pure_phases[i].phase->next_elt, pp_assemblage_ptr->pure_phases[i].moles);
		}
	}
