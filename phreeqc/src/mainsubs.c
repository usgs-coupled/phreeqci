...
/* !!!! */
	...
	if (stag_data == NULL) malloc_error();
	...
	temperature = (struct temperature *) PHRQ_malloc((size_t) sizeof (struct temperature));
 ...
 */
	...
	if (user_graph == NULL) malloc_error();
 ...
 */
	...
	if (use.solution_in == FALSE && use.mix_in == FALSE) return(FALSE);
/*
 *   Find solution
 */
	if (use.solution_in == TRUE) {
		use.solution_ptr = solution_bsearch (use.n_solution_user, &use.n_solution, FALSE);
 ...
 */
	if (use.pp_assemblage_in == TRUE) {
		use.pp_assemblage_ptr = pp_assemblage_bsearch (use.n_pp_assemblage_user, &use.n_pp_assemblage);
		if (use.pp_assemblage_ptr == NULL) {
			sprintf(error_string, "Pure phase assemblage %d not found.", use.n_pp_assemblage_user);
			error_msg(error_string, STOP);
		}
	} else {
		use.pp_assemblage_ptr = NULL;
 ...
 */
	if (use.irrev_in == TRUE) {
		...
		if (use.irrev_ptr == NULL) {
			sprintf(error_string, "Irreversible reaction %d not found.", use.n_irrev_user);
			error_msg(error_string, STOP);
		}
	} else {
		use.irrev_ptr = NULL;
 ...
 */
	if (use.exchange_in == TRUE) {
		use.exchange_ptr = exchange_bsearch(use.n_exchange_user, &use.n_exchange);
 ...
 */
	if (use.kinetics_in == TRUE) {
		use.kinetics_ptr = kinetics_bsearch(use.n_kinetics_user, &use.n_kinetics);
 ...
 */
	if (use.temperature_in == TRUE) {
		use.temperature_ptr = temperature_bsearch(use.n_temperature_user, &use.n_temperature);
		if (use.temperature_ptr == NULL) {
			sprintf(error_string, "Temperature %d not found.", use.n_temperature_user);
			error_msg(error_string, STOP);
		}
	} else {
		use.temperature_ptr = NULL;
 ...
 */
	if (use.s_s_assemblage_in == TRUE) {
		use.s_s_assemblage_ptr = s_s_assemblage_bsearch(use.n_s_s_assemblage_user, &use.n_s_s_assemblage);
		if (use.s_s_assemblage_ptr == NULL) {
			sprintf(error_string, "s_s_assemblage %d not found.", use.n_s_s_assemblage_user);
			error_msg(error_string, STOP);
		}
	} else {
		use.s_s_assemblage_ptr = NULL;
 ...
 */
	...
	for (n=0; n < count_solution; n++) {
		...
		if (solution[n] != NULL && solution[n]->new_def == TRUE) {
			...
			if (print == TRUE) {
				sprintf(token, "Initial solution %d.\t%.350s", solution[n]->n_user, solution[n]->description);
 ...
 */
	...
	for (n=0; n < count_exchange; n++) {
		...
		if (exchange[n].solution_equilibria == TRUE) {
			...
			/* free_model_allocs(); */
			if (converge == ERROR || converge1 == ERROR) { 
				error_msg("Model failed to converge for initial exchange calculation.", STOP);
			}
		} 
		for (i = n_user + 1; i <= last; i++) {
 ...
 */
	...
	for (n=0; n < count_gas_phase; n++) {
		...
		if (gas_phase[n].solution_equilibria == TRUE) {
			...
			if (converge == ERROR || converge1 == ERROR) { 
				/* free_model_allocs(); */
				error_msg("Model failed to converge for initial gas phase calculation.", STOP);
			}
 ...
 */
	...
	for (n=0; n < count_surface; n++) {
		...
		if (surface[n].solution_equilibria == TRUE) {
			...
			if (converge == ERROR || converge1 == ERROR) { 
				/* free_model_allocs(); */
				error_msg("Model failed to converge for initial surface calculation.", STOP);
			}
#endif
			...
			if (incremental_reactions == FALSE) {
				...
				if (kinetics_ptr->count_steps > 0) {
					...
					if (reaction_step > -kinetics_ptr->count_steps) {
						...
						kin_time = kinetics_ptr->steps[0] / ( (LDBLE) (-kinetics_ptr->count_steps));
					}
				} 
			}
		}
 ...
 */
	...
	if (save.pp_assemblage == TRUE) {
		...
		for(i = save.n_pp_assemblage_user + 1; i <= save.n_pp_assemblage_user_end; i++) {
	...
	if (save.s_s_assemblage == TRUE) {
		...
		for(i = save.n_s_s_assemblage_user + 1; i <= save.n_s_s_assemblage_user_end; i++) {
			s_s_assemblage_duplicate(n, i);
		}
	}
	if (save.kinetics == TRUE && use.kinetics_in == TRUE && use.kinetics_ptr != NULL) {
 ...
 */
	...
	for (i = 0; i < count_unknowns; i++) {
		if (x[i]->type == EXCH) {
			memcpy(&temp_exchange.comps[count_comps], x[i]->exch_comp, sizeof(struct exch_comp));
			temp_exchange.comps[count_comps].master = x[i]->master[0];
			temp_exchange.comps[count_comps].la = x[i]->master[0]->s->la;
			temp_exchange.comps[count_comps].formula_totals = elt_list_dup(x[i]->exch_comp->formula_totals);
 ...
 */
	...
	memcpy( (void *) temp_gas_phase.comps, (void *) use.gas_phase_ptr->comps, (size_t) count_comps * sizeof(struct gas_comp) );
/*
 *   Store in gas_phase
 */
 ...
 */
	for (i = 0; i < count_comps; i++) {
		temp_gas_phase.comps[i].moles = use.gas_phase_ptr->comps[i].phase->moles_x;
	}
/*
 *   Finish up
 */
 ...
 */
	...
	sprintf(token, "Solid solution assemblage after simulation %d.", simulation);
 ...
 */
	/* s_s_assemblage->s_s */
	temp_s_s_assemblage.s_s = (struct s_s *) PHRQ_malloc( (size_t) count_s_s * sizeof(struct s_s));
	if (temp_s_s_assemblage.s_s == NULL) malloc_error();
	for (i = 0; i < count_s_s; i++) {
		memcpy(&(temp_s_s_assemblage.s_s[i]), &(use.s_s_assemblage_ptr->s_s[i]), sizeof(struct s_s));
		/* 
		 * Malloc space for solid soution components
		 */
		count_comps = use.s_s_assemblage_ptr->s_s[i].count_comps;
		temp_s_s_assemblage.s_s[i].comps = (struct s_s_comp *) PHRQ_malloc( (size_t) count_comps * sizeof(struct s_s_comp));
		if (temp_s_s_assemblage.s_s[i].comps == NULL) malloc_error();
		memcpy( (void *) temp_s_s_assemblage.s_s[i].comps, (void *) use.s_s_assemblage_ptr->s_s[i].comps, (size_t) count_comps * sizeof(struct s_s_comp) );

		/* set initial moles for quick setup*/
		for (j = 0; j < count_comps; j++) {
			temp_s_s_assemblage.s_s[i].comps[j].initial_moles = temp_s_s_assemblage.s_s[i].comps[j].moles;
		}
	}
/*
 *   Finish up
 ...
 */
	...
	temp_pp_assemblage.next_elt = elt_list_dup(use.pp_assemblage_ptr->next_elt);
/*
 ...
 */
	...
	memcpy( (void *)  temp_pp_assemblage.pure_phases, (void *) use.pp_assemblage_ptr->pure_phases, (size_t) count_comps * sizeof(struct pure_phase) );
/*
 *   Update amounts
 */
	i = 0;
			...
			if (master_isotope[i]->moles > 0) {
				...
				if (master_isotope[i]->minor_isotope == TRUE) {
					...
					if (master_ptr->total > 0) {
						master_i_ptr->s->la = master_ptr->s->la + log10(master_i_ptr->total / master_ptr->total);
					} else {
						master_i_ptr->s->la = master_ptr->s->la;
					}
				} else if (master_isotope[i]->minor_isotope == FALSE && master_ptr->s != s_hplus && master_ptr->s != s_h2o) {
 ...
 */
		solution_ptr->totals[count_mass_balance].description = master[i]->elt->name;
 ...
 */
	if (count_isotopes_x > 0 ) {
		...
		for (i = 0; i < count_isotopes_x; i++) {
			solution_ptr->isotopes[i].total = solution_ptr->isotopes[i].master->total;
 ...
 */
	...
	/* sort only if necessary */
	if (count_solution > 1 && (solution[count_solution -1]->n_user < solution[count_solution - 2]->n_user)) {
		solution_sort();
	}
	return(OK);
}
 ...
 */
	...
	if (temp_surface.edl == FALSE) {
		...
		temp_surface.charge = (struct surface_charge *) PHRQ_malloc((size_t) (count_charge) * sizeof (struct surface_charge));
 ...
 */
	...
	for (i = 0; i < count_unknowns; i++) {
		if (x[i]->type == SURFACE) {
			memcpy(&temp_surface.comps[count_comps], x[i]->surface_comp, sizeof(struct surface_comp));
 ...
 */
	...
	exchange_ptr = exchange_bsearch(n_user, &n);
	for (i=0; i < count_master; i++) {
		if (master[i]->s->type != EX) continue;
		found = FALSE;
 ...
 */
	...
	surface_ptr = surface_bsearch(n_user, &n);
	for (i=0; i < count_master; i++) {
		if (master[i]->s->type != SURF) continue;
		...
		for (j = 0; j < surface_ptr->count_comps; j++) {
			if (surface_ptr->comps[j].rate_name != NULL) {
				for (m = 0; m < use.kinetics_ptr->count_comps; m++) {
					if (strcmp_nocase(use.kinetics_ptr->comps[m].rate_name, surface_ptr->comps[j].rate_name) != 0) continue;
					surface_ptr->charge[surface_ptr->comps[j].charge].grams =  use.kinetics_ptr->comps[m].m;
					break;
				}
			}
		}
		...
		for (j = 0; j < copy_solution.count; j++) {
			if (solution_bsearch(copy_solution.n_user[j], &n, FALSE) != NULL) {
				for (i = copy_solution.start[j]; i <= copy_solution.end[j]; i++) {
					if (i == copy_solution.n_user[j]) continue;
	...
	if (copy_pp_assemblage.count > 0) {
		for (j = 0; j < copy_pp_assemblage.count; j++) {
			if (pp_assemblage_bsearch(copy_pp_assemblage.n_user[j], &n) != NULL) {
				for (i = copy_pp_assemblage.start[j]; i <= copy_pp_assemblage.end[j]; i++) {
					if (i == copy_pp_assemblage.n_user[j]) continue;
		...
		for (j = 0; j < copy_irrev.count; j++) {
			if (irrev_bsearch(copy_irrev.n_user[j], &n) != NULL) {
				for (i = copy_irrev.start[j]; i <= copy_irrev.end[j]; i++) {
					if (i == copy_irrev.n_user[j]) continue;
		...
		for (j = 0; j < copy_mix.count; j++) {
			if (mix_bsearch(copy_mix.n_user[j], &n) != NULL) {
				for (i = copy_mix.start[j]; i <= copy_mix.end[j]; i++) {
					if (i == copy_mix.n_user[j]) continue;
		...
		for (j = 0; j < copy_surface.count; j++) {
			if (surface_bsearch(copy_surface.n_user[j], &n) != NULL) {
				for (i = copy_surface.start[j]; i <= copy_surface.end[j]; i++) {
					if (i == copy_surface.n_user[j]) continue;
		...
		for (j = 0; j < copy_temperature.count; j++) {
			if (temperature_bsearch(copy_temperature.n_user[j], &n) != NULL) {
				for (i = copy_temperature.start[j]; i <= copy_temperature.end[j]; i++) {
					if (i == copy_temperature.n_user[j]) continue;
		...
		for (j = 0; j < copy_gas_phase.count; j++) {
			if (gas_phase_bsearch(copy_gas_phase.n_user[j], &n) != NULL) {
				for (i = copy_gas_phase.start[j]; i <= copy_gas_phase.end[j]; i++) {
					if (i == copy_gas_phase.n_user[j]) continue;
		...
		for (j = 0; j < copy_kinetics.count; j++) {
			if (kinetics_bsearch(copy_kinetics.n_user[j], &n) != NULL) {
				for (i = copy_kinetics.start[j]; i <= copy_kinetics.end[j]; i++) {
					if (i == copy_kinetics.n_user[j]) continue;
	...
	if (copy_s_s_assemblage.count > 0) {
		for (j = 0; j < copy_s_s_assemblage.count; j++) {
			if (s_s_assemblage_bsearch(copy_s_s_assemblage.n_user[j], &n) != NULL) {
				for (i = copy_s_s_assemblage.start[j]; i <= copy_s_s_assemblage.end[j]; i++) {
					if (i == copy_s_s_assemblage.n_user[j]) continue;
