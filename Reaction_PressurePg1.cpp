// Reaction_PressurePg1.cpp : implementation file
//
// $Id$
//////////////////////////////////////////////////////////////////////
...
IMPLEMENT_DYNCREATE(CReaction_PressurePg1, baseCReaction_PressurePg1)


/////////////////////////////////////////////////////////////////////////////
// CReaction_PressurePg1 property page

CReaction_PressurePg1::CReaction_PressurePg1() : baseCReaction_PressurePg1(CReaction_PressurePg1::IDD)
{
	m_nType = TYPE_LIST;
	m_nLinearSteps = 1;
	//{{AFX_DATA_INIT(CReaction_PressurePg1)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
}

CReaction_PressurePg1::~CReaction_PressurePg1()
{
}

void CReaction_PressurePg1::DoDataExchange(CDataExchange* pDX)
{
	baseCReaction_PressurePg1::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CReaction_PressurePg1)
...
void CReaction_PressurePg1::DDX_Steps(CDataExchange* pDX, int nIDC)
{
	...
	if (pDX->m_bSaveAndValidate)
	{
		...
		{
			// validate Press1
			DDX_Text(pDX, IDC_E_TEMP1, m_dPress1);

			// validate Press2
			DDX_Text(pDX, IDC_E_TEMP2, m_dPress2);
...
void CReaction_PressurePg1::UpdateRadioState()
{
...
void CReaction_PressurePg1::EnableLinear(BOOL bEnable)
{
	CWnd* pWnd;
...
void CReaction_PressurePg1::EnableList(BOOL bEnable)
{
	CWnd* pWnd;
...
void CReaction_PressurePg1::InitSpin(CDataExchange* pDX)
{
	UNUSED(pDX);
...
void CReaction_PressurePg1::InitStepEditGrid(CDataExchange* pDX)
{
	UNUSED(pDX);

	// set column titles
	m_ctrlStepEditGrid.SetTextMatrix(0, 1, _T("Pressure (atm)"));
...
BEGIN_MESSAGE_MAP(CReaction_PressurePg1, baseCReaction_PressurePg1)
	//{{AFX_MSG_MAP(CReaction_PressurePg1)
...
BOOL CReaction_PressurePg1::OnInitDialog() 
{
	baseCReaction_PressurePg1::OnInitDialog();
...
void CReaction_PressurePg1::OnSetfocusRadioLinear()
{
	CString strRes;
	strRes.LoadString(IDS_RXN_PRES_732);
	m_eInputDesc.SetWindowText(strRes);
}

void CReaction_PressurePg1::OnSetfocusRadioList()
{
	CString strRes;
	strRes.LoadString(IDS_RXN_PRES_733);
	m_eInputDesc.SetWindowText(strRes);
}

void CReaction_PressurePg1::OnRadioLinear() 
{
	OnSetfocusRadioLinear();
	UpdateRadioState();
}

void CReaction_PressurePg1::OnRadioList() 
{
	OnSetfocusRadioList();
	UpdateRadioState();
}

void CReaction_PressurePg1::OnSetfocusETemp1() 
{
	CString strRes;
	strRes.LoadString(IDS_RXN_PRES_735);
	m_eInputDesc.SetWindowText(strRes);
}

void CReaction_PressurePg1::OnSetfocusETemp2() 
{
	CString strRes;
	strRes.LoadString(IDS_RXN_PRES_736);
	m_eInputDesc.SetWindowText(strRes);
}

void CReaction_PressurePg1::OnSetfocusESteps() 
{
	CString strRes;
	strRes.LoadString(IDS_RXN_PRES_737);
	m_eInputDesc.SetWindowText(strRes);
}

LRESULT CReaction_PressurePg1::OnBeginCellEdit(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CReaction_PressurePg1::OnEndCellEdit(WPARAM wParam, LPARAM lParam)
{
	NMEGINFO* pInfo = (NMEGINFO*)lParam;
	UINT nID = wParam;
	UNUSED(pInfo);
...
LRESULT CReaction_PressurePg1::OnSetfocusGrid(WPARAM wParam, LPARAM lParam)
{
	UINT nID = wParam;
	...
	case IDC_MSHFG_NUM_DESC :
		...
		break;
	case IDC_MSHFG_LIST :
		strRes.LoadString(IDS_RXN_PRES_734);
		m_eInputDesc.SetWindowText(strRes);
		break;
	default:
		ASSERT(FALSE);
...
BEGIN_EVENTSINK_MAP(CReaction_PressurePg1, baseCReaction_PressurePg1)
    //{{AFX_EVENTSINK_MAP(CReaction_PressurePg1)
	ON_EVENT(CReaction_PressurePg1, IDC_MSHFG_LIST, -602 /* KeyDown */, OnKeyDownMshfgList, VTS_PI2 VTS_I2)
	ON_EVENT(CReaction_PressurePg1, IDC_MSHFG_NUM_DESC, 71 /* EnterCell */, OnEnterCellMshfgNumDesc, VTS_NONE)
	//}}AFX_EVENTSINK_MAP
END_EVENTSINK_MAP()

void CReaction_PressurePg1::OnKeyDownMshfgList(short FAR* KeyCode, short Shift) 
{
	UNUSED(Shift);
...
void CReaction_PressurePg1::OnEnterCellMshfgNumDesc() 
{
	CString strRes;
	switch (m_egNumDesc.GetRow())
	{
	case 0:
		strRes.LoadString(IDS_STRING738);
		break;
	case 1:
		strRes.LoadString(IDS_STRING739);
		break;
	case 2:
		AfxFormatString1(strRes, IDS_DESC_STR_142, _T("pressure data") ); 
		break;
	}
...
BOOL CReaction_PressurePg1::OnHelpInfo(HELPINFO* pHelpInfo) 
{
